    ...
    : IdentityDbContext<EmaUser, IdentityRole, string>(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder builder)
    {
        ...
        builder.Entity<TradingSettings>(entity =>
        {
            ...
            entity.Property(settings => settings.PaperRiskPerTradePercent).HasPrecision(8, 4).HasDefaultValue(1m);
        ...
        builder.Entity<BacktestRun>(entity => { entity.Property(run => run.MarketDataSource).HasConversion<string>().HasMaxLength(32).HasDefaultValue(MarketDataSource.LegacyBinance); entity.Property(run => run.Symbol).HasMaxLength(64); entity.Property(run => run.Interval).HasMaxLength(8); entity.Property(run => run.MinEmaGapPercent).HasPrecision(8, 4); entity.Property(run => run.MaxStopDistancePercent).HasPrecision(8, 4); entity.Property(run => run.StartingBalanceUsdt).HasPrecision(18, 8); entity.Property(run => run.EndingBalanceUsdt).HasPrecision(18, 8); entity.Property(run => run.MarginPerTradePercent).HasPrecision(8, 4); entity.Property(run => run.Leverage).HasPrecision(8, 4); entity.Property(run => run.MaxReentryAgeBars).HasDefaultValue(6); entity.Property(run => run.ExitOnOppositeCrossover).HasDefaultValue(false); entity.Property(run => run.AccountCurrency).HasMaxLength(16); entity.Property(run => run.BrokerSymbol).HasMaxLength(64); entity.Property(run => run.HistoricalSpreadModel).HasMaxLength(128); entity.Property(run => run.HistoricalChartMode).HasMaxLength(16); entity.Property(run => run.TradeMode).HasMaxLength(16); foreach (var property in new[] { nameof(BacktestRun.CommissionPerLotPerSide), nameof(BacktestRun.ContractSize), nameof(BacktestRun.VolumeMin), nameof(BacktestRun.VolumeMax), nameof(BacktestRun.VolumeStep), nameof(BacktestRun.VolumeLimit), nameof(BacktestRun.PointSize), nameof(BacktestRun.TickSize), nameof(BacktestRun.TickValueProfit), nameof(BacktestRun.TickValueLoss), nameof(BacktestRun.StartingBalance), nameof(BacktestRun.EndingBalance), nameof(BacktestRun.GrossProfitFactor), nameof(BacktestRun.NetProfitFactor) }) entity.Property(property).HasPrecision(18, 8); entity.Property(run => run.NativeRiskPerTradePercent).HasPrecision(8, 4); entity.HasMany(run => run.Trades).WithOne(trade => trade.BacktestRun!).HasForeignKey(trade => trade.BacktestRunId).OnDelete(DeleteBehavior.Cascade); });
        builder.Entity<BacktestTrade>(entity => { entity.Property(trade => trade.AccountEquityAtEntryUsdt).HasPrecision(18, 8); entity.Property(trade => trade.MarginUsedUsdt).HasPrecision(18, 8); entity.Property(trade => trade.Leverage).HasPrecision(8, 4); entity.Property(trade => trade.SignalOpen).HasPrecision(18, 8); entity.Property(trade => trade.SignalAtr14).HasPrecision(18, 8); entity.Property(trade => trade.ReversalPowerScore).HasPrecision(8, 4); entity.Property(trade => trade.StopAnchorPrice).HasPrecision(18, 8); entity.Property(trade => trade.StopBuffer).HasPrecision(18, 8); foreach (var property in new[] { nameof(BacktestTrade.Lots), nameof(BacktestTrade.EntryBid), nameof(BacktestTrade.EntryAsk), nameof(BacktestTrade.EntrySpread), nameof(BacktestTrade.ExitBid), nameof(BacktestTrade.ExitAsk), nameof(BacktestTrade.ExitSpread), nameof(BacktestTrade.RequiredMargin), nameof(BacktestTrade.MarginUsed), nameof(BacktestTrade.AccountEquityAtEntry), nameof(BacktestTrade.EntryCommission), nameof(BacktestTrade.ExitCommission), nameof(BacktestTrade.RoundTripCommission), nameof(BacktestTrade.GrossPnl), nameof(BacktestTrade.NetPnl), nameof(BacktestTrade.InitialRiskAmount), nameof(BacktestTrade.TargetRiskAmount) }) entity.Property(property).HasPrecision(18, 8); entity.Property(trade => trade.TargetRiskPercent).HasPrecision(8, 4); entity.Property(trade => trade.ActualInitialRiskPercent).HasPrecision(8, 4); entity.HasIndex(trade => trade.BacktestRunId); entity.HasMany(trade => trade.Events).WithOne(item => item.BacktestTrade!).HasForeignKey(item => item.BacktestTradeId).OnDelete(DeleteBehavior.Cascade); });
        ...
        builder.Entity<PaperSession>(entity =>
        {
            ...
            entity.Property(session => session.PaperRiskPerTradePercent).HasPrecision(8, 4).HasDefaultValue(1m);
        ...
        builder.Entity<PaperTrade>(entity =>
        {
            ...
            entity.Property(trade => trade.TargetRiskAmount).HasPrecision(18, 8);
            entity.Property(trade => trade.TargetRiskPercent).HasPrecision(8, 4);
            entity.Property(trade => trade.ActualInitialRiskPercent).HasPrecision(8, 4);
