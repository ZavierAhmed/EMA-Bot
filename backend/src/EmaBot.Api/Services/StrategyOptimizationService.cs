    ...
    private async Task ExecuteAsync(int id, CancellationToken token)
    {
        try
        {
            ...
            foreach (var parameters in candidates)
            {
                ...
                var full = new List<(BacktestCalculation Calculation, OptimizationMetrics Metrics)>(); var development = new List<(BacktestCalculation Calculation, OptimizationMetrics Metrics)>(); var validation = new List<(BacktestCalculation Calculation, OptimizationMetrics Metrics)>();
                foreach (var symbol in symbols) foreach (var frame in frames)
                {
                    var split = run.RequestedStartUtc + TimeSpan.FromTicks((run.RequestedEndUtc - run.RequestedStartUtc).Ticks * 7 / 10);
                    // Each segment starts flat. Earlier cache candles initialize indicators and swing history only.
                    var fullCalculation = engine.RunResearch(cache[(symbol, frame)], parameters, run.RequestedStartUtc, run.RequestedEndUtc);
                    var developmentCalculation = engine.RunResearch(cache[(symbol, frame)], parameters, run.RequestedStartUtc, split);
                    var validationCalculation = engine.RunResearch(cache[(symbol, frame)], parameters, split, run.RequestedEndUtc);
                    var market = new StrategyOptimizationMarketResult { Symbol = symbol, Timeframe = frame, Full = Metrics(fullCalculation.Trades, fullCalculation.Diagnostics, run.SimulatedAccountBalanceUsdt, run.FeePercentPerSide), Development = Metrics(developmentCalculation.Trades, developmentCalculation.Diagnostics, run.SimulatedAccountBalanceUsdt, run.FeePercentPerSide), Validation = Metrics(validationCalculation.Trades, validationCalculation.Diagnostics, run.SimulatedAccountBalanceUsdt, run.FeePercentPerSide) };
                    candidate.MarketResults.Add(market); full.Add((fullCalculation, market.Full)); development.Add((developmentCalculation, market.Development)); validation.Add((validationCalculation, market.Validation)); run.CompletedWork++;
                }
                candidate.Full = Aggregate(full, run.SimulatedAccountBalanceUsdt, run.FeePercentPerSide); candidate.Development = Aggregate(development, run.SimulatedAccountBalanceUsdt, run.FeePercentPerSide); candidate.Validation = Aggregate(validation, run.SimulatedAccountBalanceUsdt, run.FeePercentPerSide);
