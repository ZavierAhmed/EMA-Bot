...
public sealed class StrategyOptimizationService(IServiceScopeFactory scopeFactory, BacktestEngine engine, TimeProvider clock, ILogger<StrategyOptimizationService> logger)
{
    ...
    public async Task<StrategyOptimizerOptions> GetOptionsAsync(CancellationToken token)
    {
        ...
        var symbols = await db.MonitoredSymbols.Where(symbol => symbol.IsEnabled && symbol.Source == MarketDataSource.Mt5Exness).OrderBy(symbol => symbol.Symbol).Select(symbol => symbol.Symbol).ToListAsync(token);
        var settings = await db.TradingSettings.AsNoTracking().SingleAsync(settings => settings.Id == 1, token);
        return new(symbols, Mt5NativeTimeframes.Supported.OrderBy(value => value).ToArray(), settings, DefaultGrid, Environment.ProcessorCount, EffectiveWorkerCount());
    }
    ...
    public async Task<StrategyOptimizationRun> StartAsync(StrategyOptimizerStartRequest request, CancellationToken token)
    {
        ...
        try
        {
            ...
            await using var scope = scopeFactory.CreateAsyncScope(); var db = scope.ServiceProvider.GetRequiredService<EmaBotDbContext>();
            var enabled = await db.MonitoredSymbols.Where(symbol => symbol.IsEnabled && symbol.Source == MarketDataSource.Mt5Exness).Select(symbol => symbol.Symbol).ToListAsync(token);
            if (normalized.Symbols.Any(symbol => !enabled.Contains(symbol, StringComparer.Ordinal))) throw new ArgumentException("Choose only currently enabled exact MT5 monitored symbols.");
            var settings = await db.TradingSettings.AsNoTracking().SingleAsync(settings => settings.Id == 1, token);
            ...
            var run = new StrategyOptimizationRun { MarketDataSource = MarketDataSource.Mt5Exness, Status = StrategyOptimizationStatus.Queued, CreatedAtUtc = clock.GetUtcNow(), RequestedStartUtc = normalized.StartUtc, RequestedEndUtc = normalized.EndUtc, SymbolsJson = JsonSerializer.Serialize(normalized.Symbols), TimeframesJson = JsonSerializer.Serialize(normalized.Timeframes), GridJson = JsonSerializer.Serialize(normalized.Grid), BaselineSettingsJson = JsonSerializer.Serialize(settings), CandidateCount = combinations.Length, MarketCount = markets, TotalWork = combinations.Length * markets, SimulatedAccountBalanceUsdt = settings.SimulatedAccountBalanceUsdt, FixedOrderSizeUsdt = settings.FixedOrderSizeUsdt, MarginPerTradePercent = settings.MarginPerTradePercent, Leverage = settings.Leverage, FeePercentPerSide = settings.FeePercentPerSide, PositionSizingMode = settings.PositionSizingMode };
    ...
    private async Task ExecuteAsync(int id, CancellationToken token)
    {
        try
        {
            if (ExecutionPhaseEnteredAsync is not null) await ExecutionPhaseEnteredAsync(id, "Queued", token); await using var scope = scopeFactory.CreateAsyncScope(); var db = scope.ServiceProvider.GetRequiredService<EmaBotDbContext>(); var historical = scope.ServiceProvider.GetRequiredService<IHistoricalMarketDataProvider>();
            ...
            var fixedSettings = JsonSerializer.Deserialize<TradingSettings>(run.BaselineSettingsJson) ?? throw new InvalidOperationException("The optimizer run does not contain a valid baseline settings snapshot.");
            ...
            token.ThrowIfCancellationRequested(); db.StrategyOptimizationTrades.AddRange(finalizationTrades.OrderBy(trade => trade.StrategyOptimizationCandidateId).ThenBy(trade => trade.Symbol).ThenBy(trade => trade.Timeframe).ThenBy(trade => trade.EntryTimeUtc).ThenBy(trade => trade.ExitTimeUtc)); run.Status = StrategyOptimizationStatus.Completed; run.CompletedAtUtc = clock.GetUtcNow(); runtimeStatuses[id] = new("Completed", null, null); await db.SaveChangesAsync(token); finalizationTimer.Stop(); logger.LogInformation("Optimizer {RunId} completed: candidate evaluation {CandidateElapsed}, ranking {RankingElapsed}, finalization {FinalizationElapsed}, {FinalizationMarkets} finalization markets, total {TotalElapsed}.", id, parallelTimer.Elapsed, rankingTimer.Elapsed, finalizationTimer.Elapsed, finalizationTotal, parallelTimer.Elapsed + rankingTimer.Elapsed + finalizationTimer.Elapsed);
        }
    ...
    internal static TradingSettings Settings(StrategyOptimizationCandidate candidate, StrategyOptimizationRun run, TradingSettings fixedSettings) => new() { Id=1,RiskReward=candidate.RiskReward,MinEmaGapPercent=candidate.MinEmaGapPercent,MaxStopDistancePercent=candidate.MaxStopDistancePercent,WaitForConfirmationCandle=candidate.WaitForConfirmationCandle,UseEma100Filter=candidate.UseEma100Filter,UseHtfRegimeFilter=candidate.UseHtfRegimeFilter,TrailingStopEnabled=candidate.TrailingStopEnabled,UseAdaptiveInitialStop=fixedSettings.UseAdaptiveInitialStop,SameTrendReentryEnabled=fixedSettings.SameTrendReentryEnabled,MaxReentryAgeBars=fixedSettings.MaxReentryAgeBars,ExitOnOppositeCrossover=fixedSettings.ExitOnOppositeCrossover,SimulatedAccountBalanceUsdt=fixedSettings.SimulatedAccountBalanceUsdt,MarginPerTradePercent=fixedSettings.MarginPerTradePercent,Leverage=fixedSettings.Leverage,FeePercentPerSide=fixedSettings.FeePercentPerSide,PositionSizingMode=fixedSettings.PositionSizingMode,FixedOrderSizeUsdt=fixedSettings.FixedOrderSizeUsdt };
