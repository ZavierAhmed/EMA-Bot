...
public sealed class Mt5HistoricalBacktestEngine(EmaSignalEngine strategy, IMt5TradeCalculator calculator, ILogger<Mt5HistoricalBacktestEngine>? logger = null)
{
    ...
    public async Task<Mt5HistoricalBacktestCalculation> RunAsync(IReadOnlyList<Mt5HistoricalExecutionBar> input, TradingSettings settings, InstrumentCatalogItem instrument, decimal commissionPerLotPerSide, string accountCurrency, StrategyMarketContext? context, CancellationToken token)
    {
        ...
        var rejectedStop = 0; var rejectedCosts = 0; var invalid = 0; var skipped = 0; var noEntry = 0; var rejectedTradeMode = 0; var rejectedInsufficientMargin = 0; var rejectedInvalidVolume = 0; var rejectedRiskBelowMinimumVolume = 0;
        ...
        foreach (var signal in evaluation.Events.Where(x => x.Status is SignalStatus.LongSignal or SignalStatus.ShortSignal).OrderBy(x => x.Time))
        {
            ...
            var sizing = await SizeAsync(instrument.Spec, direction, entry, stop.Price, equity, settings, metrics, token);
            if (!sizing.Success)
            {
            ...
            if (settings.SameTrendReentryEnabled && executed.Trade.ExitReason is BacktestExitReason.StopLoss or BacktestExitReason.TrailingStop && reenteredRegimes.Add(crossover.Time))
            {
                ...
                for (var candidate = Math.Max(occupiedUntil, 0); candidate <= lastCandidate; candidate++)
                {
                    ...
                    var reentrySizing = await SizeAsync(instrument.Spec, direction, reentryEntry, reentryStop.Price, equity, settings, metrics, token);
        ...
        var diagnostics = new BacktestDiagnostics(
            ...
            0, rejectedInsufficientMargin, rejectedInvalidVolume, rejectedTradeMode, rejectedRiskBelowMinimumVolume);
    ...
    private async Task<NativeSizingResult> SizeAsync(InstrumentSpec spec, SignalDirection direction, decimal entry, decimal initialStop, decimal equity, TradingSettings settings, EconomicsMetrics metrics, CancellationToken token)
    {
        switch (settings.PaperPositionSizingMode)
        {
            ...
            {
                var result = await new Mt5NativeRiskPositionSizer(calculator).SizeAsync(new(spec.BrokerSymbol, direction, entry, initialStop, equity, settings.PaperRiskPerTradePercent, spec.VolumeMin, spec.VolumeMax, spec.VolumeStep, spec.VolumeLimit), token);
                metrics.Calls += result.CalculationCalls;
    ...
    private void ThrowRiskSizingFailure(NativeSizingResult sizing, string brokerSymbol, SignalDirection direction, DateTimeOffset signalTimeUtc, decimal entry, decimal initialStop, decimal equity, decimal riskPercent)
    {
        ...
        if (sizing.FailureReason is NativeSizingFailure.RiskCalculationUnavailable or NativeSizingFailure.MarginCalculationUnavailable)
        {
            ...
            logger?.LogError("MT5 RiskPercent native economics unavailable. Reason={SizingFailureReason} Symbol={BrokerSymbol} Direction={Direction} SignalTimeUtc={SignalTimeUtc} Entry={Entry} InitialStop={InitialStop} Equity={Equity} RiskPercent={RiskPercent} TargetRiskAmount={TargetRiskAmount} Lots={Lots} Calls={CalculationCalls} Operation={Operation} ProviderExceptionType={ProviderExceptionType} ProviderKind={ProviderKind} RootExceptionType={RootExceptionType} SafeMessage={SafeMessage}", sizing.FailureReason, brokerSymbol, direction, signalTimeUtc, entry, initialStop, equity, riskPercent, sizing.RiskResult?.TargetRiskAmount, detail?.Lots, sizing.RiskResult?.CalculationCalls, detail?.Operation, detail?.ExceptionType, detail?.ProviderKind, detail?.RootExceptionType, detail?.SafeMessage);
