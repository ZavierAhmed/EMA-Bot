...
public sealed class BacktestEngine(EmaSignalEngine strategy)
{
    ...
    private static BacktestTrade Execute(Candle[] candles, int entryIndex, int crossoverIndex, StrategyEvent signal, SignalDirection direction, decimal entry, (decimal Price, StopSourceType Source, DateTimeOffset Time) stop, TradingSettings settings)
    {
        ...
        for (var i = entryIndex; i < candles.Length; i++)
        {
            ...
            {
                conflict = sl && tp;
                reason = sl ? BacktestExitReason.StopLoss : BacktestExitReason.TakeProfit;
            ...
            {
                var progress = direction == SignalDirection.Long ? (max - entry) / decimal.Abs(originalTp - entry) * 100m : (entry - min) / decimal.Abs(originalTp - entry) * 100m;
                var lockPercent = progress >= 100 ? 70 : progress >= 90 ? 60 : progress >= 80 ? 50 : progress >= 70 ? 40 : progress >= 60 ? 30 : progress >= 50 ? 20 : 0;
                if (lockPercent > 0)
                {
                    var nextStop = direction == SignalDirection.Long ? entry + decimal.Abs(originalTp - entry) * lockPercent / 100m : entry - decimal.Abs(originalTp - entry) * lockPercent / 100m;
                ...
                {
                    currentTp = direction == SignalDirection.Long ? entry + decimal.Abs(originalTp - entry) * 1.1m : entry - decimal.Abs(originalTp - entry) * 1.1m;
                    extended = true;
                }
            }
        }
