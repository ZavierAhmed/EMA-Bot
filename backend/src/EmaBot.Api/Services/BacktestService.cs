...
public sealed class BacktestService(EmaBotDbContext database, IBinanceHistoricalCandleService historical, TradingSettingsService settingsService, BacktestEngine engine)
{
    ...
    {
        var candles = await historical.GetRangeAsync(symbol, interval, start, end, token); var settings = await settingsService.GetAsync(token); var calculation = engine.Run(candles, settings); var trades = calculation.Trades;
        var run = new BacktestRun { Symbol = symbol, Interval = interval, RequestedStartUtc = start, RequestedEndUtc = end, ActualStartUtc = candles.FirstOrDefault()?.OpenTimeUtc, ActualEndUtc = candles.LastOrDefault()?.CloseTimeUtc, CreatedAtUtc = DateTimeOffset.UtcNow, CompletedAtUtc = DateTimeOffset.UtcNow, CandleCount = candles.Count, RiskReward = settings.RiskReward, FixedOrderSizeUsdt = settings.FixedOrderSizeUsdt, WaitForConfirmationCandle = settings.WaitForConfirmationCandle, UseEma100Filter = settings.UseEma100Filter, TrailingStopEnabled = settings.TrailingStopEnabled, FeePercentPerSide = settings.FeePercentPerSide, Status = BacktestRunStatus.Completed, Trades = trades.ToList() };
    ...
    public async Task<bool> DeleteAsync(int id, CancellationToken token) { var run = await database.BacktestRuns.FindAsync([id], token); if (run is null) return false; database.BacktestRuns.Remove(run); await database.SaveChangesAsync(token); return true; }
