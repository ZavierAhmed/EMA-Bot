...
public static class BacktestExcelExport
{
    ...
    private static IEnumerable<object?[]> SummaryRows(BacktestRun run) => Fields(
        ...
        (run.EconomicsMode == BacktestEconomicsMode.Mt5HistoricalBidAsk ? "LegacyCompatibilityStartingBalanceUsdt" : "StartingBalanceUsdt", run.StartingBalanceUsdt), ("EndingBalanceUsdt", run.EndingBalanceUsdt),
        ...
        ("NativePositionSizingMode", run.NativePositionSizingMode), ("NativeFixedLots", run.NativeFixedLots), ("NativeMarginPerTradePercent", run.NativeMarginPerTradePercent), ("NativeRiskPerTradePercent", run.NativeRiskPerTradePercent), ("NativeStartingBalance", run.StartingBalance),
        ("NativeSizingProvenance", run.EconomicsMode == BacktestEconomicsMode.Mt5HistoricalBidAsk ? run.NativePositionSizingMode is null ? "Not captured historically" : "Captured" : null),
        ("NativeSizingAuthority", run.EconomicsMode == BacktestEconomicsMode.Mt5HistoricalBidAsk ? run.NativePositionSizingMode is null ? "Native sizing mode was not captured historically; do not reconstruct it from compatibility metadata." : "Native sizing fields are authoritative for MT5 historical execution; legacy compatibility sizing fields are not." : null),
        ...
        ("InitialRiskAmountUsdtDefinition", "Legacy compatibility arithmetic. For native MT5 trades, InitialRiskAmount is broker-calculated price loss from executable entry to initial stop and is also the ActualInitialRiskAmount evidence."),
    ...
    private static IEnumerable<object?[]> SettingsRows(BacktestRun run) => Fields(
        ...
        ("LegacyCompatibilityPositionSizingMode", run.PositionSizingMode), ("LegacyCompatibilityStartingBalanceUsdt", run.StartingBalanceUsdt), ("LegacyCompatibilityMarginPerTradePercent", run.MarginPerTradePercent), ("LegacyCompatibilityLeverage", run.Leverage),
        ...
        ("EconomicsMode", run.EconomicsMode), ("AccountCurrency", run.AccountCurrency), ("BrokerSymbol", run.BrokerSymbol), ("HistoricalChartMode", run.HistoricalChartMode), ("HistoricalSpreadModel", run.HistoricalSpreadModel), ("NativePositionSizingMode", run.NativePositionSizingMode), ("NativeFixedLots", run.NativeFixedLots), ("NativeMarginPerTradePercent", run.NativeMarginPerTradePercent), ("NativeRiskPerTradePercent", run.NativeRiskPerTradePercent), ("NativeStartingBalance", run.StartingBalance), ("NativeSizingProvenance", run.EconomicsMode == BacktestEconomicsMode.Mt5HistoricalBidAsk ? run.NativePositionSizingMode is null ? "Not captured historically" : "Captured" : null), ("EndingBalance", run.EndingBalance), ("CommissionPerLotPerSide", run.CommissionPerLotPerSide), ("ContractSize", run.ContractSize), ("VolumeMin", run.VolumeMin), ("VolumeMax", run.VolumeMax), ("VolumeStep", run.VolumeStep), ("VolumeLimit", run.VolumeLimit), ("PointSize", run.PointSize), ("TickSize", run.TickSize), ("TickValueProfit", run.TickValueProfit), ("TickValueLoss", run.TickValueLoss), ("StopsLevelPoints", run.StopsLevelPoints), ("TradeMode", run.TradeMode));
    ...
    private static IEnumerable<object?[]> TradeRows(IEnumerable<BacktestTrade> trades)
    {
        var header = new object?[]
        {
            ...
            "LegacyCompatibilityPositionSizingMode", "NativePositionSizingMode", "AccountEquityAtEntryUsdt", "MarginUsedUsdt", "Leverage", "InitialStopLoss", "FinalStopLoss", "StopSourceType", "StopSourceTimeUtc", "OriginalTakeProfit", "FinalTakeProfit", "TakeProfitExtended", "ExitReason", "SameCandleExitConflict",
            ...
            "Lots", "EntryBid", "EntryAsk", "EntrySpread", "ExitBid", "ExitAsk", "ExitSpread", "RequiredMargin", "MarginUsed", "AccountEquityAtEntry", "EntryCommission", "ExitCommission", "RoundTripCommission", "GrossPnl", "NetPnl", "InitialRiskAmount", "TargetRiskPercent", "TargetRiskAmount", "ActualInitialRiskAmount", "ActualInitialRiskPercent",
        ...
        return new[] { header }.Concat(trades.Select(trade =>
        {
            ...
            return new object?[]
            {
                ...
                trade.PositionSizingMode, trade.NativePositionSizingMode, trade.AccountEquityAtEntryUsdt, trade.MarginUsedUsdt, trade.Leverage, trade.InitialStopLoss, trade.FinalStopLoss, trade.StopSourceType, trade.StopSourceTimeUtc, trade.OriginalTakeProfit, trade.FinalTakeProfit, trade.TakeProfitExtended, trade.ExitReason, trade.SameCandleExitConflict,
                ...
                trade.Lots, trade.EntryBid, trade.EntryAsk, trade.EntrySpread, trade.ExitBid, trade.ExitAsk, trade.ExitSpread, trade.RequiredMargin, trade.MarginUsed, trade.AccountEquityAtEntry, trade.EntryCommission, trade.ExitCommission, trade.RoundTripCommission, trade.GrossPnl, trade.NetPnl, trade.InitialRiskAmount, trade.TargetRiskPercent, trade.TargetRiskAmount, trade.InitialRiskAmount, trade.ActualInitialRiskPercent,
