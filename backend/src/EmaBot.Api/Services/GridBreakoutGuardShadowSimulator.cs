...
{
    public async Task<GridBreakoutGuardShadowResult> SimulateAsync(int id, CancellationToken token)
    {
        ...
        foreach (var candidate in new[] { GridBreakoutGuardCandidate.Control }.Concat(GridBreakoutGuardCandidate.Guards))
        {
            ...
            if (candidate == GridBreakoutGuardCandidate.Control) aggregate = aggregate with { FixedPathShadowProfitFactor = run.NetProfitFactor };
            aggregates.Add(aggregate);
        }
        return new(run, aggregates.AsReadOnly(), rows.AsReadOnly(), economics.Calls, cache.Count);
    }

    private static GridBreakoutGuardCandidateResult Aggregate(GridBreakoutGuardCandidate candidate,
