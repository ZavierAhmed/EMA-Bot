...
public sealed record PaperLivePnlRuntimeSnapshot(int TradeId, decimal? GrossPnl, decimal? NetPnl, decimal? PnlPercent, DateTimeOffset? CalculatedAtUtc, bool Available);
public sealed record PaperRuntimeCandle(DateTimeOffset OpenTimeUtc, DateTimeOffset CloseTimeUtc, decimal Open, decimal High, decimal Low, decimal Close);
...
public sealed record PaperSymbolRuntimeSnapshot(decimal? LatestPrice, decimal? LatestBid, decimal? LatestAsk, DateTimeOffset? LastMarketEventUtc, DateTimeOffset? LastClosedCandleUtc, PaperRuntimeCandle? FormingCandle, IndicatorSnapshot? Indicator, PaperPendingEntryRuntimeSnapshot? PendingEntry, PaperTrade? OpenTrade, SignalDirection? TrendRegimeDirection, DateTimeOffset? TrendRegimeCrossoverTimeUtc, bool ReentryEligible, bool ReentryConsumed, PaperDecisionRuntimeEvent? LastDecision, IReadOnlyList<PaperDecisionRuntimeEvent> RecentDecisions, decimal? CurrentGrossPnl, decimal? CurrentNetPnl, decimal? CurrentPnlPercent, DateTimeOffset? CurrentPnlCalculatedAtUtc, bool CurrentPnlAvailable)
{
    ...
    EmaBot.Api.Mt5Bridge.IMt5TradeCalculator? calculator = null) : IHostedService
{
    ...
    public async Task StopAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (state is not null) foreach (var runtime in state.Symbols.Values) InvalidateLivePnl(runtime);
        }
    ...
    public async Task StartSessionAsync(int sessionId, bool resume, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (!session.SameTrendReentryEnabled)
            {
                foreach (var symbol in session.Symbols) symbol.ReentryEligible = false;
                await database.SaveChangesAsync(cancellationToken);
            }
            ...
            {
                var openTrade = session.Trades.SingleOrDefault(trade => trade.PaperSessionSymbolId == symbol.Id && trade.Status == PaperTradeStatus.Open);
                var runtime = new RuntimeSymbol(symbol, openTrade);
                if (session.ExitOnOppositeCrossover && openTrade is not null && symbol.PendingOppositeExitSignalTimeUtc is { } signalTime && symbol.PendingOppositeExitDirection is { } direction && (direction is SignalDirection.Long or SignalDirection.Short) && direction != openTrade.Direction)
                    runtime.PendingOppositeExit = new(signalTime, direction);
                else
                    ClearPendingOppositeExit(symbol);
            ...
            var recoveryStarts = resume
                ? proposed.Symbols.Values.ToDictionary(runtime => runtime.Symbol.Id, runtime => runtime.LastClosedCandleUtc)
                : null;
            try
            {
                await WarmupAsync(proposed, cancellationToken);
                if (resume) await RecoverOpenTradesAfterResumeAsync(proposed, recoveryStarts!, cancellationToken);
            }
            catch
            {
            ...
            if (resume)
            {
                ...
                {
                    await AddDecisionAsync(proposed, runtime, new PaperDecisionRuntimeEvent(resumedAt, null, "SessionResumed", null, "Paper processing resumed with live MT5 market data."), cancellationToken);
                }
            }
    ...
    {
        var state = active;
        return state is null ? null : new PaperRuntimeSnapshot(state.Session.Id, state.ConnectionState, state.LastUpdateUtc, state.Symbols.ToDictionary(pair => pair.Key, pair => { var pnl = pair.Value.CurrentPnl; return new PaperSymbolRuntimeSnapshot(pair.Value.LatestPrice, pair.Value.LatestBid, pair.Value.LatestAsk, pair.Value.LastMarketEventUtc, pair.Value.LastClosedCandleUtc, pair.Value.FormingCandle, pair.Value.Indicator, PendingSnapshot(pair.Value.Pending), pair.Value.OpenTrade, pair.Value.TrendRegimeDirection, pair.Value.TrendRegimeCrossoverTimeUtc, pair.Value.ReentryEligible, pair.Value.ReentryConsumed, pair.Value.RecentDecisions.LastOrDefault(), pair.Value.RecentDecisions.AsEnumerable().Reverse().ToArray(), pnl?.GrossPnl, pnl?.NetPnl, pnl?.PnlPercent, pnl?.CalculatedAtUtc, pnl?.Available ?? false); }));
    }
    ...
    // Recovery deliberately replays closed-candle decisions only. It never invents an entry or
    // a historical fill; an eligible opposite exit remains pending until a later live Bid/Ask quote.
    private async Task RecoverOpenTradesAfterResumeAsync(RuntimeSession state, IReadOnlyDictionary<int, DateTimeOffset?> recoveryStarts, CancellationToken token)
    {
        foreach (var runtime in state.Symbols.Values)
        {
            var prior = recoveryStarts.GetValueOrDefault(runtime.Symbol.Id);
            var recovered = prior is null
                ? []
                : runtime.Candles.Where(candle => candle.CloseTimeUtc > prior.Value).OrderBy(candle => candle.CloseTimeUtc).ToArray();
            if (runtime.OpenTrade is not null && prior is not null && recovered.Length > 0)
            {
                var evaluation = strategy.Evaluate(runtime.Candles, Settings(state.Session));
                runtime.Indicator = evaluation.Snapshots.LastOrDefault();
                var opposite = FirstRecoveredOppositeSignal(evaluation.Events, prior.Value, runtime.OpenTrade.Direction);
                if (state.Session.ExitOnOppositeCrossover && runtime.PendingOppositeExit is null && opposite is not null)
                {
                    runtime.PendingOppositeExit = new PendingOppositeExit(opposite.Time, opposite.Direction);
                    await AddDecisionAsync(state, runtime, new PaperDecisionRuntimeEvent(DateTimeOffset.UtcNow, opposite.Time, "OppositeExitRecovered", opposite.Direction, $"Recovered the first valid opposite {opposite.Direction} signal during the interruption. Exit remains pending for the first reliable live executable quote."), token);
                }
            }
            if (recovered.Length > 0) runtime.LastClosedCandleUtc = recovered[^1].CloseTimeUtc;
            await PersistRuntimeSymbolAsync(runtime, token);
            var range = recovered.Length == 0 ? "no unprocessed closed candles" : $"{recovered.Length} closed candle(s) from {recovered[0].CloseTimeUtc:O} through {recovered[^1].CloseTimeUtc:O}";
            await AddDecisionAsync(state, runtime, new PaperDecisionRuntimeEvent(DateTimeOffset.UtcNow, null, "RecoveryReplay", null, $"Resume recovery evaluated {range}; no entries or historical fills were created."), token);
        }
    }
    ...
    private async Task RunStreamAsync(RuntimeSession state)
    {
        ...
        catch (MarketDataProviderException exception) when (!state.Cancellation.IsCancellationRequested && state.Session.MarketDataSource == MarketDataSource.Mt5Exness && exception.Kind is MarketDataErrorKind.Unavailable or MarketDataErrorKind.Timeout)
        {
            ...
            await UpdateSessionAsync(state.Session.Id, session => { session.Status = PaperSessionStatus.Interrupted; session.InterruptedAtUtc = interruptedAt; session.FailureMessage = "MT5 live market data was interrupted. Resume the Paper session when MT5 is connected."; });
            state.ConnectionState = "Disconnected";
            foreach (var runtime in state.Symbols.Values)
            {
                await AddDecisionAsync(state, runtime, new PaperDecisionRuntimeEvent(interruptedAt, null, "SessionInterrupted", null, "MT5 live market data was interrupted. Paper processing stopped until Resume."), CancellationToken.None);
            }
    ...
    private async Task ProcessUpdateAsync(RuntimeSession state, MarketBarUpdate update, CancellationToken token)
    {
        ...
        try
        {
            runtime.LatestPrice = update.Close; runtime.LatestBid = update.Bid; runtime.LatestAsk = update.Ask; runtime.LastMarketEventUtc = update.EventTimeUtc; runtime.FormingCandle = update.IsClosed ? null : new PaperRuntimeCandle(update.OpenTimeUtc, update.CloseTimeUtc, update.Open, update.High, update.Low, update.Close); state.LastUpdateUtc = update.EventTimeUtc;
            ...
            if (runtime.OpenTrade is null && runtime.Pending is null && runtime.ReentryEligible && !runtime.ReentryConsumed && runtime.TrendRegimeDirection is { } regime && runtime.TrendRegimeCrossoverTimeUtc is { } regimeTime && runtime.Indicator is { } snapshot && state.Session.SameTrendReentryEnabled && IsContinuation(snapshot, regime, Settings(state.Session)))
            {
                var ageBars = ReentryAgeBars(runtime.Candles, regimeTime, snapshot.Time);
                if (ageBars > state.Session.MaxReentryAgeBars)
                {
                    runtime.ReentryEligible = false;
                    await AddDecisionAsync(state, runtime, new PaperDecisionRuntimeEvent(snapshot.Time, snapshot.Time, "ReentryExpired", regime, $"Re-entry eligibility expired: regime crossover {regimeTime:O}, signal {snapshot.Time:O}, age {ageBars} bars exceeds configured maximum {state.Session.MaxReentryAgeBars} bars."), token);
                    await PersistRuntimeSymbolAsync(runtime, token);
                }
                else await ScheduleReentryAsync(state, runtime, snapshot, regime, regimeTime, ageBars, token);
            }
        }
        finally { gate.Release(); }
    }
    ...
    {
        if (runtime.OpenTrade is not null)
        {
            if (state.Session.ExitOnOppositeCrossover && signal.Direction != runtime.OpenTrade.Direction)
            {
                if (runtime.PendingOppositeExit is null)
                {
                    runtime.PendingOppositeExit = new(signal.Time, signal.Direction);
                    await PersistRuntimeSymbolAsync(runtime, token);
                    await AddDecisionAsync(state, runtime, new PaperDecisionRuntimeEvent(signal.Time, signal.Time, "OppositeExitScheduled", signal.Direction, $"Valid opposite {signal.Direction} signal scheduled the open {runtime.OpenTrade.Direction} position for closure at the first reliable executable quote."), token);
                }
                return;
            }
            await AddDecisionAsync(state, runtime, new PaperDecisionRuntimeEvent(signal.Time, signal.Time, "SkippedWhileOpen", signal.Direction, "Valid signal skipped because a position is already open."), token); await UpdateSessionAsync(state.Session.Id, session => session.SkippedWhilePositionOpen++); return;
        }
        if (runtime.Pending is not null) { await AddDecisionAsync(state, runtime, new PaperDecisionRuntimeEvent(signal.Time, signal.Time, "SkippedWhileOpen", signal.Direction, "Valid signal skipped because a pending entry already exists."), token); await UpdateSessionAsync(state.Session.Id, session => session.SkippedWhilePositionOpen++); return; }
        ...
        var index = runtime.Candles.FindIndex(candle => candle.CloseTimeUtc == crossover?.Time); var signalIndex = runtime.Candles.FindIndex(candle => candle.CloseTimeUtc == signal.Time); if (index < 1 || signalIndex < 0) return;
        var stop = InitialStopSelector.Select(runtime.Candles, index, signalIndex, signal.Snapshot, signal.Direction, Settings(state.Session)); runtime.Pending = new PendingEntry(signal.Direction, crossover!.Time, signal.Time, stop, signal.Snapshot, false, null, null);
        if (stop.UseAdaptiveInitialStop && stop.ReversalPowerScore is { } score) await AddDecisionAsync(state, runtime, new PaperDecisionRuntimeEvent(signal.Time, signal.Time, "AdaptiveStopSelected", signal.Direction, $"Adaptive initial stop selected. Reversal power {score:F1}/100 ({stop.ReversalPowerBand}), ATR14 {stop.Atr14:F2}, {(stop.Source == StopSourceType.AdaptiveSignalCandle ? "signal-candle" : "microstructure")} anchor {stop.AnchorPrice:F2}, buffer {stop.Buffer:F2}, final SL {stop.Price:F2}.", signal.Snapshot.Ema9, signal.Snapshot.Ema15, signal.Snapshot.Ema100, signal.Snapshot.GapPercent, signal.Snapshot.GapState, stop.Price, stop.Source), token);
    ...
    private async Task ScheduleReentryAsync(RuntimeSession state, RuntimeSymbol runtime, IndicatorSnapshot snapshot, SignalDirection direction, DateTimeOffset regimeTime, int ageBars, CancellationToken token)
    {
        ...
        runtime.ReentryEligible = false;
        runtime.Pending = new PendingEntry(direction, regimeTime, snapshot.Time, stop, snapshot, true, regimeTime, ageBars);
        if (stop.UseAdaptiveInitialStop && stop.ReversalPowerScore is { } score) await AddDecisionAsync(state, runtime, new PaperDecisionRuntimeEvent(snapshot.Time, snapshot.Time, "AdaptiveStopSelected", direction, $"Adaptive initial stop selected. Reversal power {score:F1}/100 ({stop.ReversalPowerBand}), ATR14 {stop.Atr14:F2}, {(stop.Source == StopSourceType.AdaptiveSignalCandle ? "signal-candle" : "microstructure")} anchor {stop.AnchorPrice:F2}, buffer {stop.Buffer:F2}, final SL {stop.Price:F2}.", snapshot.Ema9, snapshot.Ema15, snapshot.Ema100, snapshot.GapPercent, snapshot.GapState, stop.Price, stop.Source), token);
    ...
    private async Task EnterMt5PendingAsync(RuntimeSession state, RuntimeSymbol runtime, MarketBarUpdate update, CancellationToken token)
    {
        ...
        var stopTrigger = ExitExecutablePrice(pending.Direction, update.Bid, update.Ask);
        if (stopTrigger is null)
        {
            await AddDecisionAsync(state, runtime, new PaperDecisionRuntimeEvent(update.EventTimeUtc, null, "MissingBrokerEconomics", pending.Direction, "Entry rejected because the executable stop-trigger quote is unavailable.", StopPrice: pending.Stop, StopSource: pending.StopSource, Bid: update.Bid, Ask: update.Ask, EntryPrice: entry.Value), token);
            await PersistRuntimeSymbolAsync(runtime, token);
            return;
        }
        if (ExecutableStopIsBreached(pending.Direction, pending.Stop, stopTrigger.Value))
        {
            var side = pending.Direction == SignalDirection.Long ? "Bid" : "Ask";
            var relation = pending.Direction == SignalDirection.Long ? "at or below" : "at or above";
            await AddDecisionAsync(state, runtime, new PaperDecisionRuntimeEvent(update.EventTimeUtc, null, "ExecutableStopRejected", pending.Direction, $"{pending.Direction} entry rejected because the current {side} {stopTrigger.Value:F2} is already {relation} the proposed SL {pending.Stop:F2}. Entry Bid {update.Bid:F2}; Entry Ask {update.Ask:F2}; spread {update.Ask!.Value - update.Bid!.Value:F2}; executable entry {entry.Value:F2}; stop source {pending.StopSource}.", StopPrice: pending.Stop, StopSource: pending.StopSource, Bid: update.Bid, Ask: update.Ask, EntryPrice: entry.Value), token);
            await UpdateSessionAsync(state.Session.Id, session => session.RejectedByExecutableStop++);
            await PersistRuntimeSymbolAsync(runtime, token);
            return;
        }

        var target = TradeMath.InitialTarget(entry.Value, pending.Stop, pending.Direction, state.Session.RiskReward);
        if (!StopsAreValid(runtime.Symbol, pending.Direction, entry.Value, update.Bid!.Value, update.Ask!.Value, pending.Stop, target))
        {
            await AddDecisionAsync(state, runtime, new PaperDecisionRuntimeEvent(update.EventTimeUtc, null, "BrokerStopsLevelRejected", pending.Direction, "Setup rejected because the broker stop-level requirements are not satisfied using the executable stop/target quote side.", StopPrice: pending.Stop, StopSource: pending.StopSource, Bid: update.Bid, Ask: update.Ask, EntryPrice: entry.Value), token);
            await UpdateSessionAsync(state.Session.Id, session => session.InvalidStopLoss++);
            await PersistRuntimeSymbolAsync(runtime, token);
            return;
        }
        ...
        catch (PaperSizingException failure) when (failure.Kind is PaperSizingFailureKind.InvalidFixedLotsBelowMinimum or PaperSizingFailureKind.InvalidFixedLotsAboveMaximum or PaperSizingFailureKind.InvalidVolumeStep)
        {
            ...
            await UpdateSessionAsync(state.Session.Id, session => session.RejectedByInvalidVolume++);
            await PersistRuntimeSymbolAsync(runtime, token);
            return;
        }
        catch (PaperSizingException failure) when (failure.Kind == PaperSizingFailureKind.InsufficientFreeMargin)
        {
    ...
    private async Task ManageOpenTradeAsync(RuntimeSession state, RuntimeSymbol runtime, MarketBarUpdate update, CancellationToken token)
    {
        ...
        if (runtime.PendingOppositeExit is not null) { await ClosePendingOppositeExitAsync(state, runtime, update.Close, update.EventTimeUtc, token); return; }
    ...
    private async Task ManageMt5OpenTradeAsync(RuntimeSession state, RuntimeSymbol runtime, MarketBarUpdate update, CancellationToken token)
    {
        ...
        if (runtime.PendingOppositeExit is not null) { await ClosePendingOppositeExitAsync(state, runtime, exit.Value, update.EventTimeUtc, token); return; }
        if (!state.Session.TrailingStopEnabled) { QueueLivePnl(runtime, exit.Value, update.EventTimeUtc, state.Cancellation.Token); return; }
        var lockPercent = TradeMath.LockPercent(progress);
        if (lockPercent == 0m) { QueueLivePnl(runtime, exit.Value, update.EventTimeUtc, state.Cancellation.Token); return; }
        ...
        QueueLivePnl(runtime, exit.Value, update.EventTimeUtc, state.Cancellation.Token);
    }

    private void QueueLivePnl(RuntimeSymbol runtime, decimal exit, DateTimeOffset observedAt, CancellationToken token)
    {
        ...
        var trade = runtime.OpenTrade!;
        var tradeId = trade.Id; var generation = runtime.PnlGeneration; runtime.PnlCalculationGeneration = generation; var symbol = runtime.Symbol.BrokerSymbol ?? runtime.Symbol.Symbol; var direction = trade.Direction.ToString(); var lots = trade.Lots ?? 0m; var entry = trade.EntryPrice; var commission = trade.RoundTripCommission ?? 0m; var margin = trade.MarginUsed;
        _ = CalculateLivePnlAsync(runtime, tradeId, generation, symbol, direction, lots, entry, exit, commission, margin, observedAt, token);
    }
    private async Task CalculateLivePnlAsync(RuntimeSymbol runtime, int tradeId, long generation, string symbol, string direction, decimal lots, decimal entry, decimal exit, decimal commission, decimal? margin, DateTimeOffset observedAt, CancellationToken token)
    {
        try
        {
            var gross = (await Calculator().CalculateProfitAsync(new EmaBot.Api.Mt5Bridge.Mt5CalculateProfitRequest(symbol, direction, lots, entry, exit), token)).Profit;
            var net = gross - commission;
            if (runtime.PnlGeneration == generation && runtime.OpenTrade?.Id == tradeId) runtime.CurrentPnl = new PaperLivePnlRuntimeSnapshot(tradeId, gross, net, margin is > 0m ? net / margin.Value * 100m : null, observedAt, true);
        }
        catch (OperationCanceledException) { }
        catch (Exception exception)
        {
            if (runtime.PnlGeneration == generation && runtime.OpenTrade?.Id == tradeId)
            {
                var previous = runtime.CurrentPnl;
                runtime.CurrentPnl = new PaperLivePnlRuntimeSnapshot(tradeId, previous?.GrossPnl, previous?.NetPnl, previous?.PnlPercent, previous?.CalculatedAtUtc, false);
                logger.LogWarning(exception, "Live Paper P/L valuation failed for {Symbol}; trade management continues.", runtime.Symbol.Symbol);
            }
        }
        finally
        {
            if (runtime.PnlCalculationGeneration == generation)
            {
                runtime.PnlCalculationInProgress = false;
                runtime.PnlCalculationGeneration = null;
            }
        }
    }
    ...
    private async Task CloseTradeAsync(RuntimeSession state, RuntimeSymbol runtime, decimal exit, PaperExitReason reason, DateTimeOffset at, CancellationToken token)
    {
        ...
        runtime.PendingOppositeExit = null;
        runtime.ReentryEligible = state.Session.SameTrendReentryEnabled && (reason is PaperExitReason.InitialStopLoss or PaperExitReason.TrailingStop) && !trade.IsReentry;
        if (!state.Session.SameTrendReentryEnabled) runtime.ReentryConsumed = false;
        ...
        await UpdateSessionAsync(state.Session.Id, session => { session.CompletedTrades++; session.NetPnlUsdt += trade.NetPnlUsdt; session.TotalFeesUsdt += trade.TotalFeesUsdt; if (session.PositionSizingMode == PositionSizingMode.MarginPercent) { session.CurrentBalanceUsdt += trade.NetPnlUsdt; session.UsedMarginUsdt = Math.Max(0m, session.UsedMarginUsdt - (trade.MarginUsedUsdt ?? 0m)); } }); SetOpenTrade(runtime, null); await PersistRuntimeSymbolAsync(runtime, token);
    ...
    private async Task CloseMt5TradeAsync(RuntimeSession state, RuntimeSymbol runtime, decimal exit, PaperExitReason reason, DateTimeOffset at, CancellationToken token)
    {
        ...
        // Kept for compatibility with older clients. New MT5 presentation always uses the explicit computed bases.
        trade.NetPnlPercent = PaperAccounting.PnlPercentOnMargin(trade.NetPnl, trade.MarginUsed) ?? 0m;
        runtime.PendingOppositeExit = null;
        runtime.ReentryEligible = state.Session.SameTrendReentryEnabled && (reason is PaperExitReason.InitialStopLoss or PaperExitReason.TrailingStop) && !trade.IsReentry;
        if (!state.Session.SameTrendReentryEnabled) runtime.ReentryConsumed = false;
        if (runtime.ReentryEligible) await AddDecisionAsync(state, runtime, new PaperDecisionRuntimeEvent(at, null, "ReentryEligible", trade.Direction, "Position exit made one continuation re-entry eligible."), token);
        ...
        SetOpenTrade(runtime, null);
        await PersistRuntimeSymbolAsync(runtime, token);
        logger.LogInformation("MT5 Paper trade {TradeId} exited as {Reason}.", trade.Id, reason);
    }

    private async Task ClosePendingOppositeExitAsync(RuntimeSession state, RuntimeSymbol runtime, decimal exit, DateTimeOffset at, CancellationToken token)
    {
        var pending = runtime.PendingOppositeExit;
        if (pending is null || runtime.OpenTrade is null) return;
        await CloseTradeAsync(state, runtime, exit, PaperExitReason.OppositeCrossover, at, token);
        if (runtime.OpenTrade is null)
            await AddDecisionAsync(state, runtime, new PaperDecisionRuntimeEvent(at, pending.SignalTimeUtc, "OppositeCrossoverExit", pending.Direction, $"Open position exited at the first reliable executable quote after the opposite {pending.Direction} signal.", Bid: runtime.LatestBid, Ask: runtime.LatestAsk, EntryPrice: exit), token);
    }
    ...
    private static void SetOpenTrade(RuntimeSymbol runtime, PaperTrade? trade)
    {
        InvalidateLivePnl(runtime);
        runtime.OpenTrade = trade;
    }
    private static void InvalidateLivePnl(RuntimeSymbol runtime)
    {
        runtime.PnlGeneration++;
        runtime.CurrentPnl = null;
        runtime.LastPnlCalculationUtc = null;
    }
    ...
    internal static StrategyEvent? FirstRecoveredOppositeSignal(IEnumerable<StrategyEvent> events, DateTimeOffset after, SignalDirection openDirection) => events.Where(item => item.Time > after && (item.Status is SignalStatus.LongSignal or SignalStatus.ShortSignal) && item.Direction != openDirection).OrderBy(item => item.Time).FirstOrDefault();
    ...
    internal static bool ExecutableStopIsBreached(SignalDirection direction, decimal stop, decimal stopTriggerQuote) => direction == SignalDirection.Long ? stop >= stopTriggerQuote : stop <= stopTriggerQuote;
    internal static bool StopsAreValid(PaperSessionSymbol symbol, SignalDirection direction, decimal entry, decimal bid, decimal ask, decimal stop, decimal target)
    {
    ...
    {
        try { return (await Calculator().CalculateMarginAsync(new EmaBot.Api.Mt5Bridge.Mt5CalculateMarginRequest(symbol.BrokerSymbol ?? symbol.Symbol, direction.ToString(), lots, entry), token)).RequiredMargin; }
        catch (Exception exception) when (exception is not OperationCanceledException) { throw new PaperSizingException(PaperSizingFailureKind.MarginCalculationUnavailable, "Entry rejected because MT5 margin calculation is unavailable.", lots, innerException: exception); }
    }
    ...
    private static bool IsVolumeStep(decimal requested, PaperSessionSymbol symbol)
    {
        var steps = (requested - symbol.VolumeMin!.Value) / symbol.VolumeStep!.Value;
    ...
    private static TradingSettings Settings(PaperSession session) => new() { RiskReward = session.RiskReward, FixedOrderSizeUsdt = session.FixedOrderSizeUsdt, MinEmaGapPercent = session.MinEmaGapPercent, MaxStopDistancePercent = session.MaxStopDistancePercent, PositionSizingMode = session.PositionSizingMode, SimulatedAccountBalanceUsdt = session.CurrentBalanceUsdt, MarginPerTradePercent = session.MarginPerTradePercent, Leverage = session.Leverage, WaitForConfirmationCandle = session.WaitForConfirmationCandle, UseEma100Filter = session.UseEma100Filter, TrailingStopEnabled = session.TrailingStopEnabled, UseAdaptiveInitialStop = session.UseAdaptiveInitialStop, SameTrendReentryEnabled = session.SameTrendReentryEnabled, MaxReentryAgeBars = session.MaxReentryAgeBars, ExitOnOppositeCrossover = session.ExitOnOppositeCrossover, FeePercentPerSide = session.FeePercentPerSide };
    internal static int ReentryAgeBars(IReadOnlyList<Candle> candles, DateTimeOffset regimeTime, DateTimeOffset signalTime) => candles.Count(candle => candle.CloseTimeUtc > regimeTime && candle.CloseTimeUtc <= signalTime);
    ...
    private static void ClearPending(PaperSessionSymbol symbol) { symbol.PendingDirection = null; symbol.PendingCrossoverTimeUtc = null; symbol.PendingSignalTimeUtc = null; symbol.PendingStopPrice = null; symbol.PendingStopSourceType = null; symbol.PendingStopSourceTimeUtc = null; symbol.PendingSignalOpen = null; symbol.PendingSignalClose = null; symbol.PendingSignalEma9 = null; symbol.PendingSignalEma15 = null; symbol.PendingSignalEma100 = null; symbol.PendingSignalGapPercent = null; symbol.PendingSignalGapState = null; symbol.PendingIsReentry = false; symbol.PendingTrendRegimeCrossoverTimeUtc = null; symbol.PendingSignalAtr14 = null; symbol.PendingReversalPowerScore = null; symbol.PendingReversalPowerBand = null; symbol.PendingStopAnchorPrice = null; symbol.PendingStopBuffer = null; }
    private static void ClearPendingOppositeExit(PaperSessionSymbol symbol) { symbol.PendingOppositeExitSignalTimeUtc = null; symbol.PendingOppositeExitDirection = null; }
    private static void ApplyPendingOppositeExit(PaperSessionSymbol symbol, PendingOppositeExit? pending) { ClearPendingOppositeExit(symbol); if (pending is not null) { symbol.PendingOppositeExitSignalTimeUtc = pending.SignalTimeUtc; symbol.PendingOppositeExitDirection = pending.Direction; } }
    private static void ApplyPending(PaperSessionSymbol symbol, PendingEntry? pending) { ClearPending(symbol); if (pending is null) return; symbol.PendingDirection = pending.Direction; symbol.PendingCrossoverTimeUtc = pending.CrossoverTimeUtc; symbol.PendingSignalTimeUtc = pending.SignalTimeUtc; symbol.PendingStopPrice = pending.Stop; symbol.PendingStopSourceType = pending.StopSource; symbol.PendingStopSourceTimeUtc = pending.StopTimeUtc; symbol.PendingSignalOpen = pending.Snapshot.Open; symbol.PendingSignalClose = pending.Snapshot.Close; symbol.PendingSignalEma9 = pending.Snapshot.Ema9; symbol.PendingSignalEma15 = pending.Snapshot.Ema15; symbol.PendingSignalEma100 = pending.Snapshot.Ema100; symbol.PendingSignalGapPercent = pending.Snapshot.GapPercent; symbol.PendingSignalGapState = pending.Snapshot.GapState; symbol.PendingIsReentry = pending.IsReentry; symbol.PendingTrendRegimeCrossoverTimeUtc = pending.TrendRegimeCrossoverTimeUtc; symbol.PendingSignalAtr14 = pending.InitialStop.Atr14; symbol.PendingReversalPowerScore = pending.InitialStop.ReversalPowerScore; symbol.PendingReversalPowerBand = pending.InitialStop.ReversalPowerBand; symbol.PendingStopAnchorPrice = pending.InitialStop.AnchorPrice; symbol.PendingStopBuffer = pending.InitialStop.Buffer; }
    ...
    private sealed class PaperSizingException(PaperSizingFailureKind kind, string message, decimal? requestedLots = null, decimal? requiredMargin = null, decimal? availableFreeMargin = null, Exception? innerException = null) : Exception(message, innerException)
    {
        ...
        public decimal? RequestedLots { get; } = requestedLots;
        public decimal? RequiredMargin { get; } = requiredMargin;
        public decimal? AvailableFreeMargin { get; } = availableFreeMargin;
    }
}
