    ...
    ILogger<PaperTradingCoordinator> logger) : IHostedService
{
    ...
    private async Task EnterPendingAsync(RuntimeSession state, RuntimeSymbol runtime, BinanceKlineUpdate update, CancellationToken token)
    {
        ...
        var target = TradeMath.InitialTarget(update.Open, pending.Stop, pending.Direction, state.Session.RiskReward); var quantity = TradeMath.Quantity(state.Session.FixedOrderSizeUsdt, update.Open);
        var trade = new PaperTrade { PaperSessionId = state.Session.Id, PaperSessionSymbolId = runtime.Symbol.Id, Symbol = runtime.Symbol.Symbol, Interval = state.Session.Interval, Status = PaperTradeStatus.Open, Direction = pending.Direction, CrossoverTimeUtc = pending.CrossoverTimeUtc, SignalTimeUtc = pending.SignalTimeUtc, EntryTimeUtc = update.OpenTimeUtc, EntryPrice = update.Open, Quantity = quantity, EntryNotionalUsdt = update.Open * quantity, InitialStopLoss = pending.Stop, CurrentStopLoss = pending.Stop, StopSourceType = pending.StopSource, StopSourceTimeUtc = pending.StopTimeUtc, OriginalTakeProfit = target, CurrentTakeProfit = target, EntryFeeUsdt = TradeMath.Fee(update.Open, quantity, state.Session.FeePercentPerSide), TotalFeesUsdt = TradeMath.Fee(update.Open, quantity, state.Session.FeePercentPerSide), SignalClose = pending.Snapshot.Close, SignalEma9 = pending.Snapshot.Ema9, SignalEma15 = pending.Snapshot.Ema15, SignalEma100 = pending.Snapshot.Ema100, SignalGapPercent = pending.Snapshot.GapPercent, SignalGapState = pending.Snapshot.GapState };
    ...
    private async Task CloseTradeAsync(RuntimeSession state, RuntimeSymbol runtime, decimal exit, PaperExitReason reason, DateTimeOffset at, CancellationToken token)
    {
        ...
        await UpdateSessionAsync(state.Session.Id, session => { session.CompletedTrades++; session.NetPnlUsdt += trade.NetPnlUsdt; session.TotalFeesUsdt += trade.TotalFeesUsdt; }); runtime.OpenTrade = null;
    ...
    private async Task RecordClosedCandleAsync(int sessionId, RuntimeSymbol runtime, IReadOnlyList<StrategyEvent> events, CancellationToken token)
    {
        ...
        var session = await database.PaperSessions.SingleAsync(item => item.Id == sessionId, token); session.TotalCrossovers += events.Count(item => item.Status is SignalStatus.BullishCrossover or SignalStatus.BearishCrossover); session.LongSignals += events.Count(item => item.Status == SignalStatus.LongSignal); session.ShortSignals += events.Count(item => item.Status == SignalStatus.ShortSignal); session.RejectedByEma100 += events.Count(item => item.Status == SignalStatus.RejectedByEma100Filter); session.ConfirmationFailed += events.Count(item => item.Status == SignalStatus.ConfirmationFailed); await database.SaveChangesAsync(token);
    }
    ...
    private static TradingSettings Settings(PaperSession session) => new() { RiskReward = session.RiskReward, FixedOrderSizeUsdt = session.FixedOrderSizeUsdt, WaitForConfirmationCandle = session.WaitForConfirmationCandle, UseEma100Filter = session.UseEma100Filter, TrailingStopEnabled = session.TrailingStopEnabled, FeePercentPerSide = session.FeePercentPerSide };
