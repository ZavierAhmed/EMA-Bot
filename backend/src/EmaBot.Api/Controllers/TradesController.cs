...
public sealed class TradesController(EmaBotDbContext database, IHistoricalMarketDataProviderResolver historicalProviders) : ControllerBase
{
    ...
    private static bool MatchesOutcome(TradeSummaryResponse item, string outcome) => outcome.ToLowerInvariant() switch { "open" => item.Status == "Open", "win" => item.Status == "Closed" && item.NetPnl > 0, "loss" => item.Status == "Closed" && item.NetPnl < 0, "breakeven" or "break-even" => item.Status == "Closed" && item.NetPnl == 0, _ => true };
    ...
    private static TradeSummaryResponse PaperSummary(PaperTrade trade)
    {
        ...
        if (session.MarketDataSource == MarketDataSource.Mt5Exness) return new(TradeSource.Paper, trade.Id, trade.PaperSessionId, trade.Symbol, trade.Interval, trade.Status.ToString(), trade.Direction, trade.EntryTimeUtc, trade.ExitTimeUtc, trade.EntryPrice, trade.ExitPrice, trade.ExitReason?.ToString(), trade.GrossPnlUsdt, trade.NetPnlUsdt, trade.NetPnlPercent, trade.TotalFeesUsdt, null, null) { MarketDataSource = session.MarketDataSource, AccountCurrency = session.AccountCurrency, GrossPnl = trade.GrossPnl ?? 0m, NetPnl = trade.NetPnl ?? 0m, TradingCosts = trade.RoundTripCommission ?? 0m, PnlPercentBasis = "AccountEquityAtEntry" };
