...
{
    [HttpPost]
    public async Task<ActionResult<object>> Run(BacktestRequest request, CancellationToken token)
    {
        ...
        try { var run = await service.RunAsync(symbol, request.Interval, request.StartUtc, request.EndUtc, token); return CreatedAtAction(nameof(Get), new { id = run.Id }, run); }
    ...
    [HttpGet] public async Task<IActionResult> List(CancellationToken token) => Ok(await service.ListAsync(token));
    [HttpGet("{id:int}")] public async Task<IActionResult> Get(int id, CancellationToken token) => (await service.GetAsync(id, token)) is { } run ? Ok(run) : NotFound(new ApiMessage("Backtest not found."));
