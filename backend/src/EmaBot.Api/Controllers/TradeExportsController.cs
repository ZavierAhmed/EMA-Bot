...
public static class SimpleExports
{
    ...
    public static byte[] Pdf(TradeExportRow row)
    {
        ...
        var lines = new List<string> { "EMA-Bot Trade Analysis", $"Source: {row.Source}; Trade ID: {row.Id}; Parent: {row.ParentId}", $"Symbol/timeframe: {row.Symbol} {row.Interval}; Direction: {row.Direction}; Re-entry: {row.IsReentry}", $"Settings: R:R {row.RiskReward}; confirmation {row.WaitForConfirmation}; EMA100 {row.UseEma100}", $"Minimum EMA gap: {row.MinEmaGap}%; maximum stop distance: {row.MaxStopDistance}%", $"Sizing: {row.PositionSizingMode}; start balance {row.StartingBalance}; margin {row.MarginPercent}%; leverage {row.Leverage}; fee {row.FeePercent}%", $"Crossover: {row.CrossoverTime:O}; signal: {row.SignalTime:O}; signal candle {row.SignalOpen}/{row.SignalClose} {candleDirection}", $"Entry/exit: {row.EntryTime:O} / {row.ExitTime:O}; {row.Entry} / {row.Exit}", $"SL: {row.InitialStop} / {row.FinalStop}; source {row.StopSource} at {row.StopSourceTime:O}; TP: {row.OriginalTarget} / {row.FinalTarget}", $"Margin: {row.Margin}; Notional: {row.Notional}; Quantity: {row.Quantity}", $"Result: {row.ExitReason}; gross {row.GrossPnl}; fees {row.Fees}; net {row.NetPnl}; net % {row.NetPercent}; margin return {row.MarginReturnPercent}%; R {row.R}", $"EMA9/15/100: {row.Ema9}/{row.Ema15}/{row.Ema100}; gap {row.GapPercent}%", "Management timeline:" };
        lines.AddRange(row.ManagementTimeline.DefaultIfEmpty("No management events were recorded."));
        lines.Add(row.IsReentry ? "A stopped trade kept its EMA regime; a new directional continuation candle created the single allowed re-entry." : "EMA crossover strategy entry occurred at the following candle open.");
