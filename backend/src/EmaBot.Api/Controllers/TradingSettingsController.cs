...
public sealed class TradingSettingsController(TradingSettingsService settingsService, EmaBot.Api.Data.EmaBotDbContext database) : ControllerBase
{
    ...
    public async Task<ActionResult<TradingSettingsResponse>> Put(UpdateTradingSettingsRequest request, CancellationToken cancellationToken)
    {
        ...
        settings.UseAdaptiveInitialStop = request.UseAdaptiveInitialStop;
        settings.SameTrendReentryEnabled = request.SameTrendReentryEnabled;
        settings.MaxReentryAgeBars = request.MaxReentryAgeBars;
        settings.ExitOnOppositeCrossover = request.ExitOnOppositeCrossover;
        ...
        settings.PaperRiskPerTradePercent = request.PaperRiskPerTradePercent;
    ...
    internal static TradingSettingsResponse ToResponse(TradingSettings settings) => new(settings.RiskReward, settings.FixedOrderSizeUsdt, settings.MinEmaGapPercent, settings.MaxStopDistancePercent, settings.PositionSizingMode.ToString(), settings.SimulatedAccountBalanceUsdt, settings.MarginPerTradePercent, settings.Leverage, settings.WaitForConfirmationCandle, settings.UseEma100Filter, settings.UseHtfRegimeFilter, settings.TrailingStopEnabled, settings.UseAdaptiveInitialStop, settings.SameTrendReentryEnabled, settings.MaxReentryAgeBars, settings.ExitOnOppositeCrossover, settings.FeePercentPerSide, settings.PaperPositionSizingMode.ToString(), settings.PaperFixedLots, settings.PaperMarginPerTradePercent, settings.PaperRiskPerTradePercent, settings.PaperStartingBalance, settings.UpdatedAtUtc);
}

public sealed record UpdateTradingSettingsRequest(decimal RiskReward, decimal FixedOrderSizeUsdt, bool WaitForConfirmationCandle, bool UseEma100Filter, bool UseHtfRegimeFilter, bool TrailingStopEnabled, bool UseAdaptiveInitialStop = false, bool SameTrendReentryEnabled = false, int MaxReentryAgeBars = 6, bool ExitOnOppositeCrossover = false, decimal FeePercentPerSide = 0.05m, decimal MinEmaGapPercent = 0.01m, decimal MaxStopDistancePercent = 0m, PositionSizingMode PositionSizingMode = PositionSizingMode.FixedNotional, decimal SimulatedAccountBalanceUsdt = 1000m, decimal MarginPerTradePercent = 10m, decimal Leverage = 5m, PaperPositionSizingMode PaperPositionSizingMode = PaperPositionSizingMode.FixedLots, decimal PaperFixedLots = .01m, decimal PaperMarginPerTradePercent = 10m, decimal PaperRiskPerTradePercent = 1m, decimal PaperStartingBalance = 1000m);
public sealed record TradingSettingsResponse(decimal RiskReward, decimal FixedOrderSizeUsdt, decimal MinEmaGapPercent, decimal MaxStopDistancePercent, string PositionSizingMode, decimal SimulatedAccountBalanceUsdt, decimal MarginPerTradePercent, decimal Leverage, bool WaitForConfirmationCandle, bool UseEma100Filter, bool UseHtfRegimeFilter, bool TrailingStopEnabled, bool UseAdaptiveInitialStop, bool SameTrendReentryEnabled, int MaxReentryAgeBars, bool ExitOnOppositeCrossover, decimal FeePercentPerSide, string PaperPositionSizingMode, decimal PaperFixedLots, decimal PaperMarginPerTradePercent, decimal PaperRiskPerTradePercent, decimal PaperStartingBalance, DateTimeOffset UpdatedAtUtc);
