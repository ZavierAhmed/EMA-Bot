...
public static class StrategyOptimizerWorkbook
{
    ...
    private static byte[] CreateInvariant(StrategyOptimizationRun run)
    {
        ...
        {
            ("Overview", [["Run ID",run.Id.ToString()],["Status",run.Status.ToString()],["Market Data Source",MarketDataSourceLabels.For(run.MarketDataSource)],["Period",$"{run.RequestedStartUtc:O} to {run.RequestedEndUtc:O}"],["Candidates",run.CandidateCount.ToString()],["Markets",run.MarketCount.ToString()],["Recommended candidate",run.RecommendedCandidateId?.ToString() ?? "No robust candidate found"]]),
