...
public sealed class PaperSessionsController(EmaBotDbContext database, TradingSettingsService settingsService, PaperTradingCoordinator coordinator, IMarketBarStreamProvider marketBarStream, IMarketProviderCapabilities capabilities, IInstrumentCatalogProvider catalog, EmaBot.Api.Mt5Bridge.IMt5AccountReader accountReader) : ControllerBase
{
    ...
    private static PaperSessionDetailResponse ToDetail(PaperSession session, PaperRuntimeSnapshot? runtime, IReadOnlyDictionary<int, IReadOnlyList<PaperDecisionResponse>> persistedRecent)
    {
        ...
        var symbols = session.Symbols.Select(symbol =>
        {
            ...
            return new PaperSymbolResponse(symbol.Symbol, live?.LatestPrice ?? symbol.LastKnownPrice, bid, ask, bid is not null && ask is not null ? ask - bid : null, live?.LastMarketEventUtc ?? symbol.LastMarketEventUtc, live?.LastClosedCandleUtc ?? symbol.LastProcessedClosedCandleUtc, live?.LastClosedCandleUtc ?? symbol.LastProcessedClosedCandleUtc, live?.Indicator?.TrendDirection.ToString(), live?.Indicator?.Ema9, live?.Indicator?.Ema15, live?.Indicator?.Ema100, live?.Indicator?.GapPercent, live?.Indicator?.GapState.ToString(), pending?.Direction, open is null ? null : ToTrade(open, live), pending is null ? null : ToPending(pending), live?.TrendRegimeDirection ?? symbol.TrendRegimeDirection, live?.TrendRegimeCrossoverTimeUtc ?? symbol.TrendRegimeCrossoverTimeUtc, live?.ReentryEligible ?? symbol.ReentryEligible, live?.ReentryConsumed ?? symbol.ReentryConsumed, executableExit, recent.FirstOrDefault(), recent, recent.Count > 0 || live is not null);
        }).ToArray();
