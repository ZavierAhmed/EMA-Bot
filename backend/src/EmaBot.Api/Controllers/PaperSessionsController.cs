...
public sealed record PaperSessionSummaryResponse(int Id, string Interval, PaperSessionStatus Status, DateTimeOffset StartedAtUtc, int SymbolCount, int CompletedTrades, decimal NetPnlUsdt, decimal TotalFeesUsdt, string AccountCurrency, decimal NetPnl, decimal TotalTradingCosts);
public sealed record PaperDecisionResponse(DateTimeOffset TimeUtc, DateTimeOffset? CandleCloseTimeUtc, string Stage, SignalDirection? Direction, string Message, decimal? Ema9, decimal? Ema15, decimal? Ema100, decimal? GapPercent, string? GapState, decimal? StopPrice, string? StopSource, DateTimeOffset? ExpectedEntryOpenUtc, decimal? Bid, decimal? Ask, decimal? EntryPrice, decimal? Lots, decimal? RequiredMargin);
...
{
    [HttpGet]
    public async Task<IActionResult> List(CancellationToken token) => Ok((await database.PaperSessions.AsNoTracking().Include(session => session.Symbols).OrderByDescending(session => session.CreatedAtUtc).Take(30).ToListAsync(token)).Select(session => new PaperSessionSummaryResponse(session.Id, session.Interval, session.Status, session.StartedAtUtc, session.Symbols.Count, session.CompletedTrades, session.NetPnlUsdt, session.TotalFeesUsdt, session.AccountCurrency, session.NetPnl, session.TotalTradingCosts)));
    ...
    private static PaperSessionDetailResponse ToDetail(PaperSession session, PaperRuntimeSnapshot? runtime)
    {
        ...
        var symbols = session.Symbols.Select(symbol =>
        {
            ...
            return new PaperSymbolResponse(symbol.Symbol, live?.LatestPrice ?? symbol.LastKnownPrice, bid, ask, bid is not null && ask is not null ? ask - bid : null, live?.LastMarketEventUtc ?? symbol.LastMarketEventUtc, live?.LastClosedCandleUtc ?? symbol.LastProcessedClosedCandleUtc, live?.Indicator?.TrendDirection.ToString(), live?.Indicator?.Ema9, live?.Indicator?.Ema15, live?.Indicator?.Ema100, live?.Indicator?.GapPercent, live?.Indicator?.GapState.ToString(), pending?.Direction, open is null ? null : ToTrade(open), pending is null ? null : ToPending(pending), live?.TrendRegimeDirection ?? symbol.TrendRegimeDirection, live?.TrendRegimeCrossoverTimeUtc ?? symbol.TrendRegimeCrossoverTimeUtc, live?.ReentryEligible ?? symbol.ReentryEligible, live?.ReentryConsumed ?? symbol.ReentryConsumed, executableExit, live?.LastDecision is null ? null : ToDecision(live.LastDecision), live?.RecentDecisions.Select(ToDecision).ToArray() ?? [], live is not null);
        }).ToArray();
    ...
    private static PaperDecisionResponse ToDecision(PaperDecisionRuntimeEvent item) => new(item.TimeUtc, item.CandleCloseTimeUtc, item.Stage, item.Direction, item.Message, item.Ema9, item.Ema15, item.Ema100, item.GapPercent, item.GapState?.ToString(), item.StopPrice, item.StopSource?.ToString(), item.ExpectedEntryOpenUtc, item.Bid, item.Ask, item.EntryPrice, item.Lots, item.RequiredMargin);
    private static PaperTradeResponse ToTrade(PaperTrade trade) => new(trade.Id, trade.Symbol, trade.Status, trade.Direction, trade.EntryTimeUtc, trade.ExitTimeUtc, trade.EntryPrice, trade.ExitPrice, trade.Quantity, trade.InitialStopLoss, trade.CurrentStopLoss, trade.CurrentTakeProfit, trade.TakeProfitExtended, trade.BestFavorableProgressPercent, trade.GrossPnlUsdt, trade.NetPnlUsdt, trade.NetPnlPercent, trade.MfePrice, trade.MaePrice, trade.ExitReason, trade.Lots, trade.EntryBid, trade.EntryAsk, trade.EntrySpread, trade.ExitBid, trade.ExitAsk, trade.ExitSpread, trade.RequiredMargin, trade.RoundTripCommission, trade.GrossPnl, trade.NetPnl, trade.CrossoverTimeUtc, trade.SignalTimeUtc, trade.FinalStopLoss, trade.StopSourceType.ToString(), trade.StopSourceTimeUtc, trade.OriginalTakeProfit, trade.FinalTakeProfit, trade.MarginUsed, trade.AccountEquityAtEntry, trade.MfePercent, trade.MaePercent, trade.SignalOpen, trade.SignalClose, trade.SignalEma9, trade.SignalEma15, trade.SignalEma100, trade.SignalGapPercent, trade.SignalGapState.ToString(), trade.IsReentry, trade.TrendRegimeCrossoverTimeUtc);
