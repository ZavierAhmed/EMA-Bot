...
public sealed class StrategyRegimeDiagnosticsTests
{
    ...
    private sealed class CountingHistorical(IReadOnlyList<Candle> candles) : IBinanceHistoricalCandleService { private readonly Dictionary<(string Symbol,string Frame),int> requests = []; public int Count(string symbol,string frame) => requests.GetValueOrDefault((symbol,frame)); public Task<IReadOnlyList<Candle>> GetRangeAsync(string symbol,string interval,DateTimeOffset startUtc,DateTimeOffset endUtc,CancellationToken token){requests[(symbol,interval)]=Count(symbol,interval)+1;return Task.FromResult(candles);} }
}
