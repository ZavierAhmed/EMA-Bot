...
public sealed class PaperCoordinatorTests : IClassFixture<EmaBotApiFactory>
{
    ...
    public async Task SameExitCandleContinuation_EntersOnePersistedReentryAtFollowingOpen(SignalDirection direction)
    {
        ...
        Assert.False(reentry.UseAdaptiveInitialStop); Assert.True(reentry.StopSourceType is StopSourceType.Pivot or StopSourceType.FallbackLookback); Assert.Null(reentry.SignalAtr14); Assert.Null(reentry.ReversalPowerScore); Assert.Null(reentry.ReversalPowerBand); Assert.Null(reentry.StopAnchorPrice); Assert.Null(reentry.StopBuffer);
    ...
    [Fact]
    public async Task AdaptiveContinuationReentry_PersistsAdaptiveDiagnosticsThroughPendingEntry()
    {
        var coordinator = _factory.Services.GetRequiredService<PaperTradingCoordinator>();
        const decimal entry = 300m; const decimal stop = 290m; const decimal continuationClose = 310m;
        _factory.BinanceClient.Klines = TrendingWarmup(SignalDirection.Long);
        var session = await CreateSession(PaperSessionStatus.Running, openTrade: true, direction: SignalDirection.Long, entry: entry, stop: stop, adaptive: true);
        await coordinator.StartSessionAsync(session.Id, false, CancellationToken.None);
        var exitOpen = _factory.BinanceClient.Klines[^1].CloseTimeUtc.AddMilliseconds(1);
        await coordinator.ProcessUpdateForTestAsync(Kline(exitOpen, entry, stop, stop, false));
        await coordinator.ProcessUpdateForTestAsync(Kline(exitOpen, entry, continuationClose, entry - 1m, true));
        await coordinator.ProcessUpdateForTestAsync(Kline(exitOpen.AddMinutes(3), 311m, 311m, 311m, false));

        var trade = coordinator.GetRuntimeSnapshot()!.Symbols["BTCUSDT"].OpenTrade!;
        Assert.True(trade.IsReentry); Assert.True(trade.UseAdaptiveInitialStop); Assert.True(trade.StopSourceType is StopSourceType.AdaptiveSignalCandle or StopSourceType.AdaptiveMicroStructure);
        Assert.NotNull(trade.SignalAtr14); Assert.NotNull(trade.ReversalPowerScore); Assert.NotNull(trade.ReversalPowerBand); Assert.NotNull(trade.StopAnchorPrice); Assert.NotNull(trade.StopBuffer);
        await coordinator.StopSessionAsync(session.Id, CancellationToken.None);
    }
    ...
    private async Task<PaperSession> CreateSession(PaperSessionStatus status, DateTimeOffset? pendingSignal = null, bool openTrade = false, SignalDirection direction = SignalDirection.Long, decimal entry = 100m, decimal? stop = null, decimal fee = .1m, bool trailing = false, bool adaptive = false)
    {
        ...
        var session = new PaperSession { Interval = "3m", Status = status, CreatedAtUtc = DateTimeOffset.UtcNow, StartedAtUtc = DateTimeOffset.UtcNow, RiskReward = 2m, FixedOrderSizeUsdt = 100m, FeePercentPerSide = fee, TrailingStopEnabled = trailing, UseAdaptiveInitialStop = adaptive, Symbols = [symbol] };
