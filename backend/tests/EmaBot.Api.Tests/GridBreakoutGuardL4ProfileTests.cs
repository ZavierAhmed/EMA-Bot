...
public sealed class GridBreakoutGuardL4ProfileTests(Xunit.Abstractions.ITestOutputHelper output)
{
    private const string Id = GridHistoricalStrategyProfile.BreakoutGuardL4StrategyId;
    ...
    private static List<Mt5HistoricalExecutionBar> Bars(bool isLong)
    {
        ...
        // L3 precedes L4, proving the new profile remains open when the L3 guard exits.
        bars[^1] = isLong ? TelemetryFixture.Bar(52, 93.8m, 98m, 94m) : TelemetryFixture.Bar(52, 102m, 106m, 106m);
        bars.Add(isLong ? TelemetryFixture.Bar(53, 91.8m, 94m, 92m) : TelemetryFixture.Bar(53, 106m, 108m, 108m));
        return bars;
    }
    ...
    public void FrozenProfileAndRequestHaveNoEditableGuardInputs()
    {
        ...
        Assert.Same(GridHistoricalStrategyProfile.ResearchBreakoutGuardL4, p);
        Assert.Equal(new GridRangeSettings(5, 1m, 3), p.Settings);
        Assert.Equal(6, p.Settings.EmergencyStopDistanceLevels);
        Assert.Equal(GridBreakoutGuardL4Rules.Id, p.GuardId);
    ...
    [Theory]
    [InlineData(2, 1.5, 2, false)] [InlineData(4, 1.49, 2, false)]
    [InlineData(4, 1.5, 1, false)] [InlineData(4, 1.5, 2, true)]
    [InlineData(3, 1.5, 2, false)]
    ...
    public void SharedShadowAndRealPredicateParity(int level, double? delta, int adverse, bool expected)
    {
        ...
        Assert.Equal(expected, GridBreakoutGuardL4Rules.Matches(level, value, adverse));
        var candidate = GridStrictBreakoutGuardCandidate.All.Single(c => c.Id == GridBreakoutGuardL4Rules.Id);
    ...
    public async Task SameCandleL4NativeExitPreservesCounterLegsCommissionsAndTelemetry(bool isLong, InstrumentTradeMode mode)
    {
        ...
        Assert.Equal(4, basket.Legs.Count);
        ...
        Assert.Equal(new[] { 1, 1, 3, 4 }, result.Telemetry.Select(t => t.MaxFilledLevelAfterBar));
        ...
        Assert.Equal(4, native.Profits.Count(r => r.ClosePrice == basket.ExitPrice));
    ...
    public async Task NormalSameCandleExitWinsEvenWhenGuardEvidenceMatches(bool isLong, string reason)
    {
        ...
        bars[^1] = reason == "EmergencyStop" ? TelemetryFixture.Bar(53, 85m, 115m, isLong ? 92m : 108m)
            : isLong ? TelemetryFixture.Bar(53, 91.8m, 100m, 92m) : TelemetryFixture.Bar(53, 99m, 108m, 108m);
        var result = await Run(bars);
        Assert.Equal(reason, Assert.Single(result.Baskets).ExitReason.ToString());
        var t = result.Telemetry[^1];
        Assert.True(GridBreakoutGuardL4Rules.Matches(t.MaxFilledLevelAfterBar, t.AdxDeltaFromQualification, t.ConsecutiveAdverseCloses));
    ...
    public async Task InternalClosureClosesCancelsAndBlocksExactlyThreeSubsequentBars()
    {
        ...
        var cycle = e.Cycle!;
        e.CloseHistoricalResearchBreakoutGuard(TelemetryFixture.Bar(53).CloseTimeUtc, 92m);
        Assert.Equal(GridExitReason.BreakoutGuard, cycle.ExitReason);
        Assert.Equal(4, cycle.Levels.Count(l => l.Status == GridLevelStatus.Closed));
        Assert.Equal(6, cycle.Levels.Count(l => l.Status == GridLevelStatus.Canceled));
        Assert.Equal(3, e.CooldownRemaining);
        Assert.Throws<InvalidOperationException>(() => e.CloseHistoricalResearchBreakoutGuard(TelemetryFixture.Bar(53).CloseTimeUtc, 92m));
    ...
    public async Task PersistenceExcelAndShadowSourceBoundary(bool isLong)
    {
        ...
        var summary = Sheet(1);
        Assert.Contains(Id, summary); Assert.Contains(GridBreakoutGuardL4Rules.Id, summary);
    ...
    [Theory]
    [InlineData(true)] [InlineData(false)]
    public async Task CounterTwoCanExitOnSameCandleL4Fill(bool isLong)
    {
        var bars = Bars(isLong).Take(53).ToList();
        bars[^1] = isLong ? TelemetryFixture.Bar(52, 91.8m, 98m, 92m) : TelemetryFixture.Bar(52, 102m, 108m, 108m);
        var result = await Run(bars);
        ...
        Assert.Equal(4, result.Telemetry[^1].MaxFilledLevelAfterBar);
        Assert.Equal(3, result.Telemetry[^1].NewFillCount);
        Assert.Equal(4, result.Baskets[0].Legs.Count);
    }
    ...
    public async Task NativeProfitRemainsAuthoritativeAndFillMarginFailurePrecedesGuard(bool failMargin)
    {
        ...
        else
        {
            ...
            Assert.Equal(4, native.ExitCalls);
            Assert.Equal(4 * 7.123456789m, b.GrossPnl);
    ...
    private sealed class GuardNative : IMt5TradeCalculator
    {
        ...
        public async Task<Mt5ProfitCalculationPayload> CalculateProfitAsync(Mt5CalculateProfitRequest r, CancellationToken token)
        {
            ...
            if (r.ClosePrice == 92m) { ExitCalls++; return result with { Profit = 7.123456789m }; }
            return result;
        }
    ...
    [Fact]
    public void ResolverKeepsL3AndL4SeparateAndRejectsCustomRules()
    {
        Assert.Null(GridHistoricalBreakoutGuard.Resolve(null));
        Assert.Throws<ArgumentException>(() => GridHistoricalBreakoutGuard.Resolve("custom"));
        var l3 = GridHistoricalBreakoutGuard.Resolve(GridHistoricalStrategyProfile.ResearchBreakoutGuard.GuardId)!;
        var l4 = GridHistoricalBreakoutGuard.Resolve(GridHistoricalStrategyProfile.ResearchBreakoutGuardL4.GuardId)!;
        Assert.Equal("L3_ADX15_ADVERSE2", l3.Id); Assert.Equal(3, l3.MinimumFilledLevel);
        Assert.Equal(1.5m, l3.MinimumAdxDelta); Assert.Equal(2, l3.MinimumConsecutiveAdverseCloses);
        Assert.Equal("L4_ADX15_ADVERSE2", l4.Id); Assert.Equal(4, l4.MinimumFilledLevel);
        Assert.True(l3.Matches(3, 1.5m, 2)); Assert.False(l4.Matches(3, 1.5m, 2));
        var candidate = GridStrictBreakoutGuardCandidate.All.Single(c => c.Id == l4.Id);
        Assert.Equal(l4.MinimumFilledLevel, candidate.MinimumFilledLevel);
        Assert.Equal(l4.MinimumAdxDelta, candidate.MinimumAdxDelta);
        Assert.Equal(l4.MinimumConsecutiveAdverseCloses, candidate.MinimumConsecutiveAdverseCloses);
        Assert.Empty(typeof(GridHistoricalBreakoutGuard).GetConstructors());
    }

    [Theory]
    [InlineData(true, false)] [InlineData(false, false)] [InlineData(true, true)] [InlineData(false, true)]
    public async Task L3ExitsEarlierWhileL4CanRecoverOrReachL4(bool isLong, bool recover)
    {
        var bars = Bars(isLong);
        if (recover) bars[^1] = TelemetryFixture.Bar(53, 99m, 101m);
        var l3 = await Run(bars, strategy: GridHistoricalStrategyProfile.BreakoutGuardStrategyId);
        var l4 = await Run(bars);
        var early = Assert.Single(l3.Baskets); var later = Assert.Single(l4.Baskets);
        Assert.Equal(GridHistoricalExitReason.BreakoutGuard, early.ExitReason);
        Assert.Equal(3, early.Legs.Count); Assert.Equal(bars[52].CloseTimeUtc, early.ExitTime);
        Assert.Equal(recover ? GridHistoricalExitReason.TakeProfit : GridHistoricalExitReason.BreakoutGuard, later.ExitReason);
        Assert.Equal(bars[53].CloseTimeUtc, later.ExitTime);
        Assert.Null(l4.Telemetry.Single(t => t.TimeUtc == early.ExitTime).ExitReasonThisBar);
        Assert.NotEqual(early.NetPnl, later.NetPnl);
    }

    [Theory]
    [InlineData(true)] [InlineData(false)]
    public async Task SameCandleL5AlsoClosesWithEntireBasket(bool isLong)
    {
        var bars = Bars(isLong);
        bars[^1] = isLong ? TelemetryFixture.Bar(53, 89.8m, 94m, 90m) : TelemetryFixture.Bar(53, 106m, 110m, 110m);
        var native = new TelemetryFixture.Native(); var result = await Run(bars, native);
        var b = Assert.Single(result.Baskets); Assert.Equal(GridHistoricalExitReason.BreakoutGuard, b.ExitReason);
        Assert.Equal(5, b.Legs.Count); Assert.Equal(2, result.Telemetry[^1].NewFillCount);
        Assert.Equal(b.ExitTime, b.Legs[4].FillTime);
        Assert.All(b.Legs, l => Assert.Contains(native.Profits, r => r.OpenPrice == l.FillPrice && r.ClosePrice == b.ExitPrice && r.VolumeLots == l.Lots));
    }

    [Theory]
    [InlineData("GRID_RANGE_V1", 5, null)]
    [InlineData("GRID_RANGE_4L_RESEARCH_V1", 4, null)]
    [InlineData("GRID_RANGE_BREAKOUT_GUARD_RESEARCH_V1", 5, 3)]
    [InlineData("GRID_RANGE_BREAKOUT_GUARD_L4_RESEARCH_V1", 5, 4)]
    public async Task PersistedExcelMetadataAndShadowEligibilityFollowExactProfile(string strategy, int levels, int? guardLevel)
    {
        var result = await new GridHistoricalBacktestEngine(new TelemetryFixture.Native()).RunAsync(Bars(true),
            TelemetryFixture.Request(levels) with { StrategyId = strategy }, TelemetryFixture.Instrument);
        await using var db = new EmaBotDbContext(new DbContextOptionsBuilder<EmaBotDbContext>().UseInMemoryDatabase(Guid.NewGuid().ToString()).Options);
        var run = GridBacktestPersistence.Map(result, DateTimeOffset.UtcNow); db.GridBacktestRuns.Add(run); await db.SaveChangesAsync(); db.ChangeTracker.Clear();
        using var zip = new ZipArchive(new MemoryStream((await GridBacktestExcelExport.CreateAsync(db, run.Id, default))!.Bytes));
        XElement[] Rows(int n) { using var stream = zip.GetEntry($"xl/worksheets/sheet{n}.xml")!.Open(); return XDocument.Load(stream).Descendants().Where(e => e.Name.LocalName == "row").ToArray(); }
        var summary = Rows(1).ToDictionary(r => r.Elements().First().Value, r => r.Elements().Last().Value);
        var headers = Rows(4)[0].Elements().Select(c => c.Value).ToArray();
        var legacy = typeof(GridLegResponse).GetProperties().Select(p => p.Name);
        Assert.Equal(guardLevel is null ? legacy : legacy.Concat(new[] { "ExitReason", "ExitTimeUtc", "ExitPrice" }), headers);
        if (guardLevel is { } level)
        {
            Assert.Equal($"L{level}_ADX15_ADVERSE2", summary["Breakout guard"]);
            Assert.Equal(level.ToString(), summary["Monitor from level"]);
            Assert.Equal("1.5", summary["Minimum ADX increase"]); Assert.Equal("2", summary["Minimum consecutive adverse closes"]);
            Assert.Equal(result.Baskets.Count(b => b.ExitReason == GridHistoricalExitReason.BreakoutGuard).ToString(), summary["BreakoutGuardExits"]);
        }
        else Assert.False(summary.ContainsKey("Breakout guard"));
        var native = new TelemetryFixture.Native(); var simulator = new GridBreakoutGuardShadowSimulator(db, native);
        if (guardLevel is null) Assert.Equal(8, (await simulator.SimulateAsync(run.Id, default)).Candidates.Count);
        else
        {
            var error = await Assert.ThrowsAsync<GridGuardResearchException>(() => simulator.SimulateAsync(run.Id, default));
            Assert.Contains("requires an unguarded historical Grid profile", error.Message); Assert.Empty(native.Profits);
        }
        if (strategy == GridRangeSettings.StrategyId) Assert.Equal(6, (await simulator.SimulateStrictAsync(run.Id, default)).Candidates.Count);
        else
        {
            var calls = native.Profits.Count;
            var error = await Assert.ThrowsAsync<GridGuardResearchException>(() => simulator.SimulateStrictAsync(run.Id, default));
            Assert.Contains("requires a telemetry-enabled GRID_RANGE_V1", error.Message); Assert.Equal(calls, native.Profits.Count);
        }
    }
