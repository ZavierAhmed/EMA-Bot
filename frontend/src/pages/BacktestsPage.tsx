...
import { ApiError, downloadBacktestExcel, getBacktests, getMonitoredSymbols, runBacktest } from '../api'

const intervals = ['3m', '5m', '15m', '30m', '1h', '2h', '4h', '6h', '8h', '12h', '1d', '1w', '1M']
const backtestTimeoutMilliseconds = 130_000
...
export function BacktestsPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!symbol || !start || !end || busy) return
    ...
    try {
      ...
      setError(controller.signal.aborted ? 'Backtest timed out while waiting for MT5 historical data. Check the MT5 bridge/history state and retry.' : requestError instanceof Error ? requestError.message : 'Backtest failed.')
    } finally {
  ...
  return <div className="space-y-8">
    <div>
      ...
      <p className="mt-2 text-slate-600">New research uses MT5 / Exness bars. Economics remain the legacy compatibility model until broker-aware sizing and costs are introduced.</p>
    </div>
    <form onSubmit={submit} className="grid gap-4 rounded-lg border border-slate-200 bg-white p-5 md:grid-cols-4">
      ...
      <button disabled={busy} className="rounded bg-slate-950 px-4 py-2 text-sm font-medium text-white disabled:opacity-50 md:col-span-4">{busy ? 'Running backtest...' : 'Run Backtest'}</button>
    </form>
...
function Result({ run, exporting, exportExcel }: { run: BacktestRun; exporting: boolean; exportExcel: (id: number) => Promise<void> }) {
  return <section className="rounded-lg border border-slate-200 bg-white p-5">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-semibold">{run.symbol} · {run.interval}</h2><p className="mt-2 text-sm text-amber-800">Market data: {run.marketDataSourceLabel} · Economics: Legacy compatibility model</p></div><button type="button" disabled={exporting} onClick={() => void exportExcel(run.id)} className="rounded border px-3 py-2 text-sm disabled:opacity-50">{exporting ? 'Exporting…' : 'Export Excel'}</button></div>
