...
export function BacktestsPage() {
  ...
  useEffect(() => { if (!symbol) { setEconomics(null); return } void getMt5HistoricalBacktestEconomicsPreview(symbol).then(setEconomics).catch(value => setEconomics({ ready: false, reason: value instanceof Error ? value.message : 'MT5 economics preview is unavailable.', brokerSymbol: symbol, accountCurrency: null, startingBalance: null, sizingMode: null, fixedLots: null, marginPerTradePercent: null, commissionPerLotPerSide: null, historicalSpreadModel: null, chartMode: null, contractSize: null, volumeMin: null, volumeMax: null, volumeStep: null, stopsLevelPoints: null, pointSize: null })) }, [symbol])
...
function EconomicsPreview({ value }: { value: Mt5HistoricalBacktestEconomicsPreview | null }) { if (!value) return <p className="text-sm text-slate-500 md:col-span-4">Select an MT5 instrument to inspect native historical economics.</p>; if (!value.ready) return <p role="alert" className="text-sm text-amber-800 md:col-span-4">MT5 Historical Economics unavailable: {value.reason}</p>; return <div className="rounded border border-slate-200 bg-slate-50 p-3 text-xs md:col-span-4"><p className="font-semibold">MT5 Historical Economics</p><p className="mt-1">Market data: MT5 / Exness · Broker: {value.brokerSymbol} · Account currency: {value.accountCurrency} · Starting balance: {value.startingBalance}</p><p>Sizing: {value.sizingMode === 'FixedLots' ? `${value.fixedLots} lots` : `${value.marginPerTradePercent}% margin`} · Commission: {value.commissionPerLotPerSide}/lot/side · Chart: {value.chartMode}</p><p>Spread: {value.historicalSpreadModel} · Point: {value.pointSize} · Contract: {value.contractSize} · Volume: {value.volumeMin}/{value.volumeMax}/{value.volumeStep} · Stops level: {value.stopsLevelPoints}</p></div> }
