...
import { ApiError, downloadTradeExcel, downloadTradePdf, getMonitoredSymbols, getTrade, getTradeChart, getTrades, type ExnessDemoTradeDetail, type NativeBacktestTradeDetail, type TradeChartData, type TradeDetail, type TradeSummary } from '../api'
...
function Mt5PaperDetails({ detail }: { detail: TradeDetail }) {
  const nativeBacktest = detail.summary.source === 'Backtest' && (detail as NativeBacktestTradeDetail).economicsMode === 'Mt5HistoricalBidAsk' ? detail as NativeBacktestTradeDetail : null
  const sizing = nativeBacktest ? nativeBacktest.nativePositionSizingMode === null ? 'Not captured historically' : nativeBacktest.nativePositionSizingMode : detail.paperPositionSizingMode ?? '—'
  const currency = detail.summary.accountCurrency
  const outcome = detail.summary.status === 'Open' ? 'Unresolved / Open' : `Gross ${format(detail.summary.grossPnl)} ${currency} · Total explicit commission costs ${format(detail.summary.tradingCosts)} ${currency} · Net ${format(detail.summary.netPnl)} ${currency}`
  const entrySide = detail.summary.direction === 'Long' ? 'Ask' : 'Bid'; const exitSide = detail.summary.direction === 'Long' ? 'Bid' : 'Ask'
  return <div className="grid min-w-0 gap-4 lg:grid-cols-2"><section className="min-w-0 rounded-lg border border-slate-200 bg-white p-4 text-sm"><h2 className="font-semibold">MT5 {nativeBacktest ? 'historical backtest' : 'Paper'} trade details</h2><dl className="mt-3 grid gap-2 text-slate-700"><div><dt className="font-medium text-slate-950">Entry economics</dt><dd>{date(detail.summary.entryTimeUtc)} at {format(detail.summary.entryPrice)} · trigger {entrySide} · Bid {format(detail.entryBid)}, Ask {format(detail.entryAsk)}, spread {format(detail.entrySpread)}</dd></div><div><dt className="font-medium text-slate-950">Native sizing</dt><dd>{sizing} · lots {format(detail.lots)} · quantity (contract units) {format(detail.quantity)} · required margin {format(detail.requiredMargin)} {currency} · entry equity {format(detail.accountEquityAtEntry)} {currency}</dd></div><div><dt className="font-medium text-slate-950">Risk levels</dt><dd>Initial SL {format(detail.initialStopLoss)} ({detail.stopSourceType}); initial risk {format(detail.initialRiskAmount)} {currency}; final SL {format(detail.finalStopLoss)}. Original TP {format(detail.originalTakeProfit)}; final TP {format(detail.finalTakeProfit)}</dd></div><div><dt className="font-medium text-slate-950">Exit economics</dt><dd>{date(detail.summary.exitTimeUtc)} at {format(detail.summary.exitPrice)} · trigger {exitSide} · Bid {format(detail.exitBid)}, Ask {format(detail.exitAsk)}, spread {format(detail.exitSpread)} · {detail.summary.exitReason ?? 'Open'}</dd></div><div><dt className="font-medium text-slate-950">Result</dt><dd>{outcome}. {detail.summary.status === 'Open' ? 'Final P/L is unavailable until this Paper position is resolved.' : `P/L % on margin: ${format(detail.summary.pnlPercentOnMargin)}%. Account return: ${format(detail.summary.accountReturnPercent)}%.`} R {format(detail.summary.netRMultiple)}</dd></div></dl><p className="mt-3 text-slate-600">The signal scheduled entry for the exact next bar. The Paper position entered on the first executable Ask quote for Long or Bid quote for Short observed on that bar. Spread cost is embedded in those executable fills and is not separately deducted.</p></section><section className="min-w-0 rounded-lg border border-slate-200 bg-white p-4 text-sm"><h2 className="font-semibold">Management timeline</h2><div className="mt-3 space-y-2">{detail.events.map((event, index) => <p key={`${event.type}-${index}`}><strong>{event.type}</strong> · {date(event.timeUtc)} · price {format(event.marketPrice)}{event.newStop !== null ? ` · SL ${format(event.newStop)}` : ''}{event.newTakeProfit !== null ? ` · TP ${format(event.newTakeProfit)}` : ''}</p>)}</div><p className="mt-3 text-slate-500">Source: {nativeBacktest ? 'Backtest' : 'Paper'} / MT5 Exness · session #{detail.summary.parentId}</p></section></div>
}
