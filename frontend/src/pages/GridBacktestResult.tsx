...
export function GridBacktestResult({ detail, busy, exportExcel, exportGuardResearch }: { detail: GridBacktestDetail; busy: boolean; exportExcel: () => void; exportGuardResearch: () => void }) {
  const r = detail.run
  ...
  return <section className="space-y-5 rounded-lg border border-slate-200 bg-white p-5">
    <div className="flex flex-wrap justify-between gap-3"><div><h2 className="font-semibold">{r.strategyId} · {r.symbol} · {r.interval}</h2><p className="text-sm text-slate-600">MT5 / Exness · {r.accountCurrency} · {r.requestedStartUtc.slice(0, 10)} – {r.requestedEndUtc.slice(0, 10)}</p></div><button disabled={busy} onClick={exportExcel} className="rounded border px-3 py-2 text-sm disabled:opacity-50">{busy ? 'Exporting…' : 'Export Grid Excel'}</button>{!guarded && <button disabled={busy} onClick={exportGuardResearch} className="rounded border px-3 py-2 text-sm disabled:opacity-50">Export Guard Research</button>}</div>
