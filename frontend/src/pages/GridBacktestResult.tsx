...
export function GridBacktestResult({ detail, busy, exportExcel, exportGuardResearch, exportStrictGuardResearch }: { detail: GridBacktestDetail; busy: boolean; exportExcel: () => void; exportGuardResearch: () => void; exportStrictGuardResearch: () => void }) {
  const r = detail.run
  const guardLevel = r.strategyId === 'GRID_RANGE_BREAKOUT_GUARD_L4_RESEARCH_V1' ? 4 : 3
  const guarded = r.strategyId === 'GRID_RANGE_BREAKOUT_GUARD_RESEARCH_V1' || r.strategyId === 'GRID_RANGE_BREAKOUT_GUARD_L4_RESEARCH_V1'
  ...
  return <section className="space-y-5 rounded-lg border border-slate-200 bg-white p-5">
    <div className="flex flex-wrap justify-between gap-3"><div><h2 className="font-semibold">{r.strategyId} · {r.symbol} · {r.interval}</h2><p className="text-sm text-slate-600">MT5 / Exness · {r.accountCurrency} · {r.requestedStartUtc.slice(0, 10)} – {r.requestedEndUtc.slice(0, 10)}</p></div><button disabled={busy} onClick={exportExcel} className="rounded border px-3 py-2 text-sm disabled:opacity-50">{busy ? 'Exporting…' : 'Export Grid Excel'}</button>{!guarded && <button disabled={busy} onClick={exportGuardResearch} className="rounded border px-3 py-2 text-sm disabled:opacity-50">Export Guard Research</button>}{r.strategyId === 'GRID_RANGE_V1' && <button disabled={busy} onClick={exportStrictGuardResearch} className="rounded border px-3 py-2 text-sm disabled:opacity-50">Export Strict Guard Research</button>}</div>
    {!guarded && <p className="text-xs text-slate-500">Guard research is SCREENING ONLY: fixed-path shadow exits; future sizing, qualification and later baskets are not rerun.</p>}
    <div className="rounded border border-slate-200 bg-slate-50 p-4 text-sm"><h3 className="font-semibold">Frozen Grid strategy</h3>{r.strategyId !== 'GRID_RANGE_V1' && <p className="mt-2 font-semibold text-amber-800">RESEARCH ONLY · Not approved for Paper/Demo/Live.</p>}{guarded && <p className="mt-2">Breakout guard: L{guardLevel}_ADX15_ADVERSE2 · Monitor from L{guardLevel} · ADX increase ≥ 1.5 · Consecutive adverse closes ≥ 2 · Completed-candle exit; future cycles use updated equity.</p>}<p className="mt-2">{r.levelCount} equal-lot levels · {r.gridBasketRiskPercent}% total basket price-risk · {r.rangeLookback}-bar range · ATR{r.atrPeriod} × {r.atrSpacingMultiplier} · ADX{r.adxPeriod} ≤ {r.adxThreshold} · anchor TP · emergency stop one spacing beyond Level {r.levelCount} (stop level {r.levelCount + 1}) · {r.cooldownBars}-bar cooldown · no martingale</p><p className="mt-2 text-slate-600">Commission: {r.commissionPerLotPerSide} {r.accountCurrency}/lot/side · {r.tradeMode} · Bid chart with captured spread · drawdown uses realized basket balances.</p></div>
