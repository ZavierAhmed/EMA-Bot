...
export type TradeDetail = { summary: TradeSummary; crossoverTimeUtc: string; signalTimeUtc: string; quantity: number; entryNotionalUsdt: number; initialStopLoss: number; finalStopLoss: number; stopSourceType: string; stopSourceTimeUtc: string; originalTakeProfit: number; finalTakeProfit: number; takeProfitExtended: boolean; entryFeeUsdt: number; exitFeeUsdt: number | null; mfePrice: number; mfePercent: number; maePrice: number; maePercent: number; signalClose: number; signalEma9: number | null; signalEma15: number | null; signalEma100: number | null; signalGapPercent: number | null; signalGapState: string; riskReward: number; fixedOrderSizeUsdt: number; waitForConfirmationCandle: boolean; useEma100Filter: boolean; trailingStopEnabled: boolean; feePercentPerSide: number; events: TradeManagementEvent[]; hasDetailedManagementHistory: boolean }
