...
export type PaperTrade = { id: number; symbol: string; status: string; direction: string; crossoverTimeUtc: string; signalTimeUtc: string; entryTimeUtc: string; exitTimeUtc: string | null; entryPrice: number; exitPrice: number | null; quantity: number; initialStopLoss: number; currentStopLoss: number; finalStopLoss: number | null; stopSourceType: string; stopSourceTimeUtc: string; originalTakeProfit: number; currentTakeProfit: number; finalTakeProfit: number | null; takeProfitExtended: boolean; bestFavorableProgressPercent: number; grossPnlUsdt: number; netPnlUsdt: number; netPnlPercent: number; mfePrice: number; mfePercent: number | null; maePrice: number; maePercent: number | null; exitReason: string | null; lots: number | null; entryBid: number | null; entryAsk: number | null; entrySpread: number | null; exitBid: number | null; exitAsk: number | null; exitSpread: number | null; requiredMargin: number | null; marginUsed: number | null; accountEquityAtEntry: number | null; roundTripCommission: number | null; grossPnl: number | null; netPnl: number | null; signalOpen: number | null; signalClose: number; signalEma9: number | null; signalEma15: number | null; signalEma100: number | null; signalGapPercent: number | null; signalGapState: string; isReentry: boolean; trendRegimeCrossoverTimeUtc: string | null; currentGrossPnl: number | null; currentNetPnl: number | null; currentPnlPercent: number | null; currentPnlCalculatedAtUtc: string | null; currentPnlAvailable: boolean; useAdaptiveInitialStop: boolean; signalAtr14: number | null; reversalPowerScore: number | null; reversalPowerBand: string | null; stopAnchorPrice: number | null; stopBuffer: number | null }
...
export type PaperSession = { id: number; interval: string; status: string; startedAtUtc: string; stoppedAtUtc: string | null; interruptedAtUtc: string | null; failureMessage: string | null; riskReward: number; fixedOrderSizeUsdt: number; minEmaGapPercent: number; maxStopDistancePercent: number; waitForConfirmationCandle: boolean; useEma100Filter: boolean; trailingStopEnabled: boolean; useAdaptiveInitialStop: boolean; feePercentPerSide: number; totalCrossovers: number; longSignals: number; shortSignals: number; rejectedByEma100: number; rejectedByEmaGap: number; rejectedByStopDistance: number; rejectedByFees: number; rejectedByTradingCosts: number; rejectedByInsufficientMargin: number; confirmationFailed: number; invalidStopLoss: number; skippedWhilePositionOpen: number; completedTrades: number; netPnlUsdt: number; totalFeesUsdt: number; connectionState: string; lastUpdateUtc: string | null; symbols: PaperSymbol[]; recentTrades: PaperTrade[]; marketDataSource: string; accountCurrency: string; paperPositionSizingMode: PaperPositionSizingMode; paperFixedLots: number; paperMarginPerTradePercent: number; startingBalance: number; currentBalance: number; usedMargin: number; netPnl: number; totalTradingCosts: number }
