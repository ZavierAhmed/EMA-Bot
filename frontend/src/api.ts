...
export type PaperTrade = { id: number; symbol: string; status: string; direction: string; entryTimeUtc: string; exitTimeUtc: string | null; entryPrice: number; exitPrice: number | null; quantity: number; initialStopLoss: number; currentStopLoss: number; currentTakeProfit: number; takeProfitExtended: boolean; bestFavorableProgressPercent: number; grossPnlUsdt: number; netPnlUsdt: number; netPnlPercent: number; mfePrice: number; maePrice: number; exitReason: string | null; lots: number | null; entryBid: number | null; entryAsk: number | null; entrySpread: number | null; exitBid: number | null; exitAsk: number | null; exitSpread: number | null; requiredMargin: number | null; roundTripCommission: number | null; grossPnl: number | null; netPnl: number | null }
export type PaperSymbol = { symbol: string; latestPrice: number | null; latestBid: number | null; latestAsk: number | null; latestSpread: number | null; lastMarketEventUtc: string | null; lastClosedCandleUtc: string | null; trend: string | null; ema9: number | null; ema15: number | null; ema100: number | null; gapPercent: number | null; gapState: string | null; pendingDirection: string | null; openTrade: PaperTrade | null }
